...
// (items array with per-item status). No file-buffer failover: on DB error the
// client gets a 503 (documented slice-1 limitation for segment queues).
...
// If the acks' leaseId belongs to a v2 lease (registry hit), handles the whole
// batch and returns true; otherwise returns false and the caller proceeds v1.
