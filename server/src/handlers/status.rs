...
pub async fn handle_status() -> Response {
    ...
    // The literal above is an object; `as_object_mut` cannot fail, and the
    // fallback keeps that unreachable branch from being an `unwrap` in a handler.
    if let Some(fields) = body.as_object_mut() {
        if let Some(kafka) = kafka {
            fields.insert("kafka".to_string(), kafka);
        }
        if let Some(sqs) = sqs {
