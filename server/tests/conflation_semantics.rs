...
async fn configure(h: &Http, queue: &str, options: Value) -> Result<(), String> {
    let (code, v) = h
        .req(
            "POST",
            "/api/v1/configure",
            Some(&json!({"queue": queue, "options": options})),
        )
        .await?;
...
async fn push(h: &Http, items: Vec<Value>) -> Result<Vec<Value>, String> {
    let (code, v) = h
        .req("POST", "/api/v1/push", Some(&json!({"items": items})))
        .await?;
    if !(200..300).contains(&code) {
    ...
    let arr = v
        .as_array()
        .cloned()
        .ok_or_else(|| format!("push result not an array: {v}"))?;
    for r in &arr {
...
async fn pop_queue(h: &Http, queue: &str, qs: &str) -> Result<(u16, Value), String> {
    h.req("GET", &format!("/api/v1/pop/queue/{queue}?{qs}"), None)
        .await
}
...
async fn pop_part(h: &Http, queue: &str, part: &str, qs: &str) -> Result<(u16, Value), String> {
    h.req(
        "GET",
        &format!("/api/v1/pop/queue/{queue}/partition/{part}?{qs}"),
        None,
    )
    .await
}
...
async fn ack_batch_completed(
    ...
    let (code, v) = h
        .req(
            "POST",
            "/api/v1/ack/batch",
            Some(&json!({"acknowledgments": acks, "consumerGroup": group})),
        )
        .await?;
    if code != 200 {
    ...
    v.as_array()
        .cloned()
        .ok_or_else(|| format!("ack/batch result not an array: {v}"))
}
...
async fn wipe_segments(c: &Client, pid: &str) -> Result<(), String> {
    c.execute(
        "DELETE FROM queen.log_segments WHERE partition_id = $1::text::uuid",
        &[&pid],
    )
    .await
    .map_err(|e| format!("wipe segments: {e}"))?;
    c.execute(
        "DELETE FROM queen.log_txns WHERE partition_id = $1::text::uuid",
        &[&pid],
    )
    .await
...
async fn c01_tail_only_delivery_and_cursor_jump(h: &Http, c: &Client) -> Case {
    ...
    let v = pop_ok(
        h,
        &q,
        &format!("consumerGroup={g}&subscriptionMode=all&conflation=true&batch=200"),
    )
    .await?;
    let m = msgs(&v);
    chk!(
    ...
    chk!(
        msg_n(&m[0]) == 99,
        "the one delivered message must be the TAIL (n=99), got n={}",
        msg_n(&m[0])
    );
    chk!(
    ...
    chk!(
        ...
        "pop must not move committed (it is the ack that jumps): {row}"
    );
    chk!(
    ...
    chk!(
        a.get("success") == Some(&Value::Bool(true)),
        "clean ack must succeed: {a}"
    );
    chk!(
    ...
    let row = consumer_must(c, &q, "Default", g).await?;
    chk!(
        ci64(&row, "committed") == 99,
        "ack must jump committed to the tail (99): {row}"
    );
    chk!(
        cnull(&row, "batch_end"),
        "the lease must be closed (batch_end NULL): {row}"
    );
    chk!(
...
async fn c02_the_guarantee(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        m.len() == 1,
        "conflating pop must deliver exactly one message, got {}",
        m.len()
    );
    chk!(
        msg_n(&m[0]) == 4,
        "must deliver the tail T (n=4), got n={}",
        msg_n(&m[0])
    );
    ...
    chk!(
        m.len() == 1,
        "the follow-up pop must deliver exactly one message, got {}",
        m.len()
    );
    chk!(
        msg_n(&m[0]) == 7,
    ...
    chk!(
        a.get("success") == Some(&Value::Bool(true)),
        "second ack must succeed: {a}"
    );
    let row = consumer_must(c, &q, "Default", g).await?;
    chk!(
        ci64(&row, "committed") == 7,
        "second ack lands on the new tail: {row}"
    );
    Ok(())
}
...
async fn c03_retry_supersession_budget_dlq(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        m.len() == 1,
        "conflating pop must deliver one message, got {}",
        m.len()
    );
    chk!(
        msg_n(&m[0]) == 2,
        "must deliver the tail (n=2), got n={}",
        msg_n(&m[0])
    );
    let a = ack(
        h,
    ...
    chk!(
        a.get("success") == Some(&Value::Bool(true)),
        "failed-ack must be accepted: {a}"
    );
    chk!(
        a.get("dlq") != Some(&Value::Bool(true)),
        "first failure must NOT dead-letter: {a}"
    );
    let row = consumer_must(c, &q, "Default", g).await?;
    chk!(
        ci64(&row, "committed") == -1,
        "a nack keeps the cursor (§7.1.3): {row}"
    );
    chk!(
        ci64(&row, "batch_retry_count") == 1,
        "one explicit failed = one charge: {row}"
    );
    ...
    chk!(
        m.len() == 1,
        "redelivery pop must deliver one message, got {}",
        m.len()
    );
    chk!(
        msg_n(&m[0]) == 3,
    ...
    chk!(
        m.len() == 1,
        "third delivery must be one message, got {}",
        m.len()
    );
    chk!(
        msg_n(&m[0]) == 4,
        "third delivery is the newest tail (n=4), got n={}",
        msg_n(&m[0])
    );
    let a = ack(
        h,
    ...
    chk!(
        dl[0].1 == 2,
        "the DLQ row's retry_count is the exhausted budget (2): {dl:?}"
    );
    let row = consumer_must(c, &q, "Default", g).await?;
    chk!(
        ci64(&row, "committed") == 4,
    ...
    chk!(
        ci64(&row, "batch_retry_count") == 0,
        "DLQ filing resets the budget: {row}"
    );
    ...
    chk!(
        m.len() == 1 && msg_n(&m[0]) == 5,
        "after the DLQ the group resumes on the next tail: {v}"
    );
    let a = ack(
        h,
        &s(&m[0], "transactionId"),
...
async fn c04_mode_composition(h: &Http, c: &Client) -> Case {
    ...
    let v = pop_ok(
        h,
        &q,
        "consumerGroup=live&subscriptionMode=new&conflation=true",
    )
    .await?;
    chk!(
        msgs(&v).is_empty(),
    ...
    let v = pop_ok(
        h,
        &q,
        "consumerGroup=rebuild&subscriptionMode=all&conflation=true&batch=200",
    )
    .await?;
    let m = msgs(&v);
    chk!(
    ...
    chk!(
        msg_n(&m[0]) == 32,
        "and it is the newest (n=32), got n={}",
        msg_n(&m[0])
    );
    let a = ack(
        h,
    ...
    chk!(
        ci64(&row, "committed") == 32,
        "rebuild cursor lands on the tail: {row}"
    );
    Ok(())
}
...
async fn c05_refusals(h: &Http) -> Case {
    ...
    let (code, v) = pop_queue(
        h,
        &q,
        "consumerGroup=g&subscriptionMode=all&conflation=true",
    )
    .await?;
    chk!(
        code == 200,
        "control: a conflating pop WITH a group must be served, got {code}: {v}"
    );
...
async fn c06_conflict_stored_wins(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        msgs(&v).is_empty(),
        "registration pop on an empty queue delivers nothing: {v}"
    );
    chk!(
...
async fn c07_sizing_batch_partitions(h: &Http, c: &Client) -> Case {
    ...
    let items: Vec<Value> = (0..8)
        .map(|i| push_item(&q, &format!("P{i:02}"), json!({"n": i})))
        .collect();
    push(h, items).await?;
    let v = pop_ok(
        h,
        &q,
        &format!("consumerGroup={g}&subscriptionMode=all&conflation=true&batch=8"),
    )
    .await?;
    let m = msgs(&v);
    chk!(
    ...
    chk!(
        pids.len() == 8,
        "the 8 messages must come from 8 DISTINCT partitions, got {}",
        pids.len()
    );
    chk!(
    ...
    for r in ack_batch_completed(h, g, &acks).await? {
        chk!(
            r.get("success") == Some(&Value::Bool(true)),
            "leg-1 ack failed: {r}"
        );
    }
    chk!(
        pending_partitions(c, &q, g).await? == 0,
        "leg 1 must be fully drained"
    );
    ...
    let items: Vec<Value> = (0..70)
        .map(|i| push_item(&q2, &format!("P{i:02}"), json!({"n": i})))
        .collect();
    push(h, items).await?;
    ...
    chk!(
        total == 70,
        "every partition's tail must be delivered exactly once: got {total}/70"
    );
    chk!(
        largest == 64,
...
async fn c08_depth_fields(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        msgs(&v).is_empty(),
        "registration pop delivers nothing: {v}"
    );
    ...
    chk!(
        msgs(&v).len() == 1,
        "A's conflating lease delivers one tail: {v}"
    );

    let (code, d) = h
        .req(
            "GET",
            &format!("/api/v1/resources/queues/{q}/depth?group={g}"),
            None,
        )
        .await?;
    ...
    chk!(
        parts.len() == 3,
        "the per-partition array keeps listing all 3 partitions: {d}"
    );
    let a = parts
        .iter()
        .find(|p| p.get("partition").and_then(Value::as_str) == Some("A"));
    chk!(
    ...
    let (code, v) = pop_part(
        ...
        &q,
        "B",
        &format!("consumerGroup={audit}&subscriptionMode=all"),
    )
    .await?;
    chk!(
        code == 200 && msgs(&v).len() == 1,
        "audit leases B: {code}: {v}"
    );
    let (code, regular) = h
        .req(
            "GET",
            &format!("/api/v1/resources/queues/{q}/depth?group={audit}"),
            None,
        )
        .await?;
...
async fn c09_pinned_pop_registers_metadata(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        m.len() == 1,
        "the single message is delivered, got {}",
        m.len()
    );
...
async fn c10_default_off_regression(h: &Http, c: &Client) -> Case {
    ...
    let v = pop_ok(
        h,
        &q,
        &format!("consumerGroup={g}&subscriptionMode=all&batch=100&partitions=2"),
    )
    .await?;
    let m = msgs(&v);
    chk!(
    ...
    for part in ["A", "B"] {
        ...
        chk!(
            ...
            "{part}: full span acked → committed = 2: {row}"
        );
        chk!(
            ci64(&row, "total_consumed") == 3,
            "{part}: total_consumed = 3: {row}"
        );
        chk!(
            ci64(&row, "batch_retry_count") == 0,
            "{part}: untouched budget: {row}"
        );
...
async fn c14_empty_partition_seal(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        ci64(&row, "committed") == -1,
        "budget-free retry keeps the cursor: {row}"
    );
...
async fn c15_window_buffer_gate(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        ci64(&row, "committed") == -1,
        "and must not move the cursor: {row}"
    );
    ...
    chk!(
        !m.is_empty(),
        "after the window the partition serves again: {v}"
    );
    let newest = m.iter().map(msg_n).max().unwrap_or(-1);
    chk!(
        newest == 2,
        "and the newest (n=2) is among the delivered, got max n={newest}"
    );
    Ok(())
}
...
async fn c16_concurrency_one_worker_per_partition(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        drained,
        "the swarm must drain all {PARTS} partitions within the deadline"
    );
    Ok(())
}
...
async fn c17_boot_idempotence(broker: &mut BrokerProc, c: &Client) -> Case {
    broker.restart().await.map_err(|e| {
        format!(
            "second boot must re-apply the schema cleanly (the DROP \
...
async fn c18_transaction_wire(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        m.len() == 1,
        "conflating pop delivers one message, got {}",
        m.len()
    );
    chk!(
        msg_n(&m[0]) == 4,
    ...
    chk!(
        !results.is_empty()
            && results
                .iter()
                .all(|r| r.get("success") == Some(&Value::Bool(true))),
    ...
    chk!(
        m.len() == 1,
        "the bundled push must be poppable from {qb}: {v}"
    );
    chk!(
...
async fn c20_empty_conflict_still_echoes(h: &Http, c: &Client) -> Case {
    ...
    let row = cgm_row(c, &q, g)
        .await?
        .ok_or("the wildcard pop registers the group")?;
    chk!(
...
async fn c21_discovery_echo_reports_what_was_applied(h: &Http, c: &Client) -> Case {
    ...
    chk!(
        m.len() == 5,
        "a plain discovery pop delivers the full span, got {}: {v}",
        m.len()
    );
    chk!(
    ...
    let row = cgm_row(c, &q, g)
        .await?
        .ok_or("the discovery pop registers the group")?;
    chk!(
        row.get("conflation") == Some(&Value::Bool(false)),
        "stored policy is OFF: {row}"
    );
...
async fn c22_pop_maintenance_keeps_the_echo(h: &Http) -> Case {
    ...
    for (label, path) in [
        (
            "wildcard",
            format!("/api/v1/pop/queue/{q}?consumerGroup=g&conflation=true"),
        ),
        (
            "pinned",
            format!("/api/v1/pop/queue/{q}/partition/Default?consumerGroup=g&conflation=true"),
        ),
        (
            "discovery",
            format!("/api/v1/pop?namespace=nope&task=nope&consumerGroup=g&conflation=true"),
        ),
    ] {
    ...
    let off = h
        .req(
            "GET",
            &format!("/api/v1/pop/queue/{q}?consumerGroup=g2"),
            None,
        )
        .await?;
    ...
    chk!(
        (200..300).contains(&code),
        "disable pop maintenance -> {code}: {v}"
    );
    out
}
...
async fn c23_wildcard_sp_reads_the_stored_policy(h: &Http, c: &Client) -> Case {
    ...
    let v = pop_ok(
        h,
        &q,
        &format!("consumerGroup={g}&subscriptionMode=all&batch=1"),
    )
    .await?;
    let m = msgs(&v);
    ...
    let v = pop_ok(
        h,
        &q2,
        &format!("consumerGroup={g2}&subscriptionMode=all&conflation=true"),
    )
    .await?;
    let m = msgs(&v);
...
async fn conflation_semantics() {
    ...
    report.push((
        "tail_only_delivery_and_cursor_jump",
        c01_tail_only_delivery_and_cursor_jump(&h, &c).await,
    ));
    report.push(("the_guarantee", c02_the_guarantee(&h, &c).await));
    report.push((
        "retry_supersession_budget_dlq",
        c03_retry_supersession_budget_dlq(&h, &c).await,
    ));
    report.push(("mode_composition", c04_mode_composition(&h, &c).await));
    report.push(("refusals_400", c05_refusals(&h).await));
    report.push((
        "conflict_stored_wins",
        c06_conflict_stored_wins(&h, &c).await,
    ));
    report.push((
        "sizing_batch_partitions",
        c07_sizing_batch_partitions(&h, &c).await,
    ));
    report.push(("depth_fields", c08_depth_fields(&h, &c).await));
    report.push((
        "pinned_pop_registers_metadata",
        c09_pinned_pop_registers_metadata(&h, &c).await,
    ));
    report.push((
        "default_off_regression_guard",
        c10_default_off_regression(&h, &c).await,
    ));
    report.push((
        "empty_pop_echoes_conflation",
        c11_empty_pop_echoes_conflation(&h).await,
    ));
    report.push((
        "delayed_all_deferred_guard",
        c12_delayed_all_deferred(&h, &c).await,
    ));
    report.push((
        "delayed_partial_newest_visible",
        c13_delayed_partial(&h, &c).await,
    ));
    report.push((
        "empty_partition_seal_guard",
        c14_empty_partition_seal(&h, &c).await,
    ));
    report.push((
        "window_buffer_gate_guard",
        c15_window_buffer_gate(&h, &c).await,
    ));
    report.push((
        "concurrency_one_worker_per_partition_guard",
        c16_concurrency_one_worker_per_partition(&h, &c).await,
    ));
    report.push(("transaction_wire", c18_transaction_wire(&h, &c).await));
    report.push((
        "pinned_first_group_is_seeded",
        c19_pinned_first_group_is_seeded(&h, &c).await,
    ));
    report.push((
        "empty_conflict_still_echoes",
        c20_empty_conflict_still_echoes(&h, &c).await,
    ));
    report.push((
    ...
    report.push((
        "pop_maintenance_keeps_the_echo",
        c22_pop_maintenance_keeps_the_echo(&h).await,
    ));
    // Last on purpose: it kills and re-spawns the broker.
    report.push((
        "boot_idempotence",
        c17_boot_idempotence(&mut broker, &c).await,
    ));
