...
--
-- The trigger FUNCTIONS live in 019_worker_metrics.sql with the rest of the
-- metrics machinery. Only the attachment is here, and it is here for two
-- reasons that are easy to get wrong:
--
-- 1. APPLY ORDER. Under the old apply order the metrics file ran before the
--    log schema created queen.log_partitions. An attachment sitting there and
--    guarded on the table existing was therefore skipped entirely on the FIRST
--    boot of a fresh database, and only landed on the second — so a new install
--    silently reported zero partitions created for one process lifetime. This
--    file runs after both 001_log_schema (the table) and 019_worker_metrics
--    (the trigger functions), so everything the attachment needs is always
--    there.
--
...
--    REPLACE FUNCTION in 019_worker_metrics rebinds it with no lock on the
--    table. If you ever
...
-- partitions allocated by push (003_log_push's provisioning INSERT), and
-- partitions_deleted counts the retention cleanup reclaiming empty ones
-- (queen.log_partition_cleanup_step_v1, 006_log_maintenance). Before 2026-07-30 both were
