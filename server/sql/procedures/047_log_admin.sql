...
--   * 027_storage_v2_observability.sql — the v2 branches of the
...
--
-- Two `AND quv.storage = 'segments'` joins SURVIVE, in get_consumer_groups_v4
-- and list_messages_v1, and they are deliberate: in list_messages_v1 the join
-- also supplies namespace/task/priority, and in get_consumer_groups_v4 dropping
-- it would newly surface cursors whose queue has no queen.queues row at all.
-- They are queue-existence guards now, not engine selectors.
--
...
-- The 010/027 rows pipeline that used to fill result.messages ahead of the log
...
-- The 010/027 rows leg of the UNION (over the dropped dead-letter/message
...
-- ============================================================================
-- 011's queen.record_trace_v1 resolved the trace's message_id from the rows
...
-- (applied AFTER 011 in lexical order, the same versioning pattern the other
-- redefinitions here use). The lookup is now gone with its table: the trace is
...
-- FKs would reject a log trace. They are dropped here (idempotent; first done
-- by the retired 030) — the coordination table is preserved, decoupled from the
-- engine-specific stores. Traces stay keyed by (partition_id, transaction_id).
-- ============================================================================
...
-- [] forever for log queues (their cursors live in queen.log_consumers). The
-- 008 rows CTE that used to run first, prepending its own (always empty) array,
...
DECLARE
    v_log JSONB;
BEGIN
    -- The 008 rows CTE (consumer_lag, over the dropped coordination/message
...
-- Folded in from the retired 099 (same reason as above: the 008 original joined
-- the rows partition tables only, so a log-engine group would render as {}).
-- That 008 body — merged in front of this one via jsonb `||` and always empty —
-- is deleted along with its tables.
...
DECLARE
    v_log JSONB;
BEGIN
    -- The 008 rows body (partition_data, over the dropped coordination/message
