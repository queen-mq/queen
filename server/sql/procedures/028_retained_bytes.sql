...
-- bytes per queue = SUM(octet_length(blob)) over live segments — an unqualified
...
--   * write ONLY retained_bytes — never last_computed_at (that stamp is the
--     counters cycle's freshness signal), never the prev_* rate trio.
-- ============================================================================
...
BEGIN
    ...
    SELECT COALESCE(array_agg(t.id    ORDER BY t.id), '{}'),
      ...
      INTO v_queue_ids, v_bytes
      FROM (
        SELECT q.id, COALESCE(sb.retained_bytes, 0)::bigint AS bytes
        FROM queen.queues q
        LEFT JOIN (
            SELECT lp.queue_id,
    ...
    UPDATE queen.stats st
      ...
      FROM unnest(v_queue_ids, v_bytes) AS u(queue_id, bytes)
