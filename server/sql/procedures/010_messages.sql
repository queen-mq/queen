...
--     047_log_admin.sql redefines both (CREATE OR REPLACE, applied after the
--     0xx range), so these copies were overwritten at every boot and never
--     ran. The live definitions are the ones in 047.
...
-- 044/047 address log_dlq elsewhere; every consumer group's snapshot for that
...
-- (list_messages_v1 / get_dlq_messages_v1 are granted by 047, which owns the
--  live definitions; get_message_v1 no longer exists.)
