...
-- (048_log_stats.sql), which writes the 'queue' rows straight from the log
...
-- the rows-engine partitions table. 048_log_stats.sql (applied after this file)
-- DROPs the legacy (TEXT) form and defines the live (TEXT, UUID) one over the
-- log tables.
        ...
        SELECT jsonb_build_object(
            'namespaces', COALESCE(jsonb_agg(
                jsonb_build_object(
                    ...
                    -- while the queue actually had N log partitions. Joined on
                    -- the (tenant_id, name) queue identity, so it agrees with
                    -- the per-queue 'partitions' get_queues_v2 reports.
                    'partitions', (
                        SELECT COUNT(*)
                        FROM queen.log_partitions lp
                        JOIN queen.log_queues lq ON lq.id = lp.queue_id
                        JOIN queen.queues q2
                          ON q2.name = lq.name AND q2.tenant_id = lq.tenant_id
        ...
        SELECT jsonb_build_object(
            'tasks', COALESCE(jsonb_agg(
                jsonb_build_object(
                    ...
                    -- gave a phantom 0/1 per queue, not the live count).
                    'partitions', (
                        SELECT COUNT(*)
                        FROM queen.log_partitions lp
                        JOIN queen.log_queues lq ON lq.id = lp.queue_id
...
-- tables, and 048_log_stats.sql redefines it (TEXT, UUID) over the log tables.
...
-- in 048_log_stats.sql replaces it. is_stats_leader went with it: the Rust
...
-- 048_log_stats.sql, which applies after this file -- granting them here would
