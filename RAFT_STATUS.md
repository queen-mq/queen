...
| PERF-L level-1 fusion — drain the channel into one entry | **done** (this commit) | `QUEEN_RAFT_DRAIN_GREEDY` (default **on**; 0 = one command per select wake) | **The C1000 win Alice predicted: fill the entry, don't deepen the pipeline (O23/§ D4).** The batcher's arrival arm drains the whole pending channel into the cycle's batch before planning, so one log entry (one fsync) carries many commands instead of ~one per wake. Order preserved; `drain_batch` still applies the caps; no I3/I1/I2 change (rsm:: **383 passed / 0 failed**). **Numbers** (VM root@164.90.215.224, binary md5-12 `a33172d9516c`, round-4 knobs, one-knob A/B): **C1000** push p50 **188 → 20.4 ms**, p99 **750 → 43.3 ms** (postgres same box/loader 15 / 33), commands-per-entry **4.5 → 9.7**, fsyncs/s **~600 → 279** at the same ~2700 appends/s, CPU flat, RSS **500 → 222 MB**, drained, ERRLINES=0. **A20k** p50 **6.11 → 4.96**, p99 **17.3 → 14.7**, CPU down. **FAT100** 246k → 239k msg/s (within this saturated shape's single-run variance; off itself 9% under the 271k anchor). Headroom remains (9.7 « the 4096 cap; deeper packing needs a larger in-flight bound = the D4/phase-3 decision). **Files**: `server/src/rsm/batcher.rs`. |
| PERF-N drop the redundant Value parse in the push handler | **done** (this commit) | none (unconditional) | **The receiver-JSON lever (Alice's residual #1), measured.** The push handler's name-length guard parsed the whole body into a `serde_json::Value` tree only to read the queue/partition names, then the facade re-parsed the same bytes into its borrowed `PushBodyIn<'a>` (payload = `&RawValue`); the guard now parses a borrowed names-only struct that skips the payload, so serde builds no tree. Facade re-checks each name (accept/reject unchanged). **Numbers** (VM 164.90.215.224, FAT100 60 s, same box, OLD `a33172d9516c` vs NEW): `Value::deserialize` **8.2% → absent**, `BTreeMap::insert_entry` **3.3% → absent**, allocator incl. **46.9% → 40.8%**, throughput **241k → 255k msg/s (+5.5%)**, push p50 **1581 → 1483 ms**, p99 **2114 → 1958**, ERRLINES 0. Functional identical on both binaries: normal **201**, over-long name **413** `name_too_long`, malformed **400**. **Residual**: the remaining allocator cost is `push_impl`'s per-item `String` churn (queue/partition/txn clones + the `(String,String,String)` HashMap) and the store/frame write path, not JSON — the next receiver lever if wanted. **Not run**: the difffuzz oracle (error-string parity beyond the 3 codes). **Files**: `server/src/handlers/raft.rs`. |
