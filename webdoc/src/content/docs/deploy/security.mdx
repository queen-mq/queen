...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): main.rs's three new routes, auth.rs's explicit read-only arm for POST /api/v1/resources/kv/list, and the proxy's reclassification of the two replay routes to queue admin. This page states no route classes, so nothing on it moved.
---
