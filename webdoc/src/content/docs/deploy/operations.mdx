...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): the retention paragraph is rewritten for the /configure merge rule; switches.rs and schema.sql changed only comments.
---
...
whole-segment, so a window is a floor on what is kept, not a ceiling. Since 1.6.0 `/configure`
merges, so a later call that omits these keys leaves retention exactly as it is; the two ways to
turn it back off are both explicit: send `retentionEnabled: false` (or `null`, which restores the
default), or send the whole configuration with `"mode": "replace"`, which is what
`queenctl apply -f` does. See
