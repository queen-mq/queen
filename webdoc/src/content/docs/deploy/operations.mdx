...
## Maintenance mode suspends the timer fire

<Aside type="danger">
While push maintenance mode is on, pushes are diverted to the broker's disk spool, and **a timer fire
cannot be diverted**. The fire is a `DELETE` of the staging row and a push of the frame in the *same*
transaction, and that shared commit is the whole of its exactly-once guarantee; putting the push half
on the spool would break it. So the fire suspends for the duration of the window.

Nothing is lost and nothing is duplicated. The timers stay in `queen.log_timers`, the backlog drains
oldest first when the window closes, and the visible symptom in the meantime is
`queen_timers_oldest_late_seconds` climbing. Read that as a delay, not as message loss, and check
`GET /api/v1/system/maintenance` before concluding the sweeper is broken.
</Aside>
