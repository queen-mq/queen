...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): main.rs registers three new routes and no new environment variable; nothing this page describes moved.
---
...
Nothing is installed into either required part. The DDL and the 33 stored-procedure files are
...
tools travel inside the same image, and so do the two wire-protocol facades and the lake sink:
`queen-kafka`, `queen-sqs` and `queen-s3` sit beside the broker binary and stay inert until
`QUEEN_KAFKA_EMBEDDED`, `QUEEN_SQS_EMBEDDED` or `QUEEN_S3_EMBEDDED` says otherwise.
...
<CardGrid>
  ...
  <LinkCard
    href="/deploy/kafka"
    title="Kafka facade"
    description="queen-kafka in the same image: the advertised-address footgun, TLS and SASL, and cluster mode."
  />
  <LinkCard
    href="/deploy/sqs"
    title="SQS facade"
    description="queen-sqs in the same image: SigV4 credentials, and the one variable a second replica needs."
  />
  <LinkCard
    href="/deploy/s3"
    title="S3 sink"
    description="queen-s3 in the same image: the log into any S3-compatible bucket as JSONL or Parquet, exactly once, every variable and the bucket policy."
  />
</CardGrid>
