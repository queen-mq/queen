...
tools travel inside the same image, and so do the two wire-protocol facades: `queen-kafka` and
`queen-sqs` sit beside the broker binary and stay inert until `QUEEN_KAFKA_EMBEDDED` or
`QUEEN_SQS_EMBEDDED` says otherwise.
