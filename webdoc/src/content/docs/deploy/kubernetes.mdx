...
- **`PG_PORT` points at PostgreSQL directly, not at a pooler.** The broker holds a session-scoped
  advisory lock across the boot schema apply and keeps a transaction open across every retention
  cycle whose one job is holding that cycle's lock, plus the statements the hot paths prepare once
  per connection, and all of that lives on one specific backend session.
