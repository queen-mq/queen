...
- **Dead Letter**, `/dlq`: the dead-letter listing, a failure breakdown that filters it, per-row
  purge, and a bulk purge of every record for one queue, optionally narrowed to one consumer group
...
this console does not call it yet, so purge is the only action on a row. The bulk purge takes its
scope from the queue and consumer-group filters alone: the error breakdown narrows the table, not
the cleanup, so a purge by criteria deletes every record for that queue and cannot be undone. And an unknown is not a
...
<Screenshot
  src="dashboard-dlq"
  alt="The dead letter view. A failure breakdown heads the page: one thin bar divides the page's messages by error text, and under it two columns list each error with its count, beside a link to the ones past the top six. Below it a table lists each dead-lettered message with its queue, consumer group, error text, retry count, time of failure and a single Purge action."
  caption="The breakdown is computed from the messages on this page, not from a broker-side total, and every row and bar segment narrows the table below it. Purge is the only action on a row."
/>
