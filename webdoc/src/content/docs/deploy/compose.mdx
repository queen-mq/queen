...
the front door and not around it. Drop the key and the proxy answers `401`; send a `Host` whose
first label names no cluster and it answers `421`; reach a broker directly without a token and the
broker answers `401`.
