...
verifiedBy:
  - test/run.sh
digest: >-
  Queen MQ's conformance evidence is `test/run.sh`: seven client suites (JavaScript, Python, Go,
  Rust, C++, PHP and the queenctl CLI) on three topologies each, plus the broker's unit tests,
  an HA mesh assertion, a two-tenant isolation suite and raw-HTTP kv/timer and conflation
  suites, every cell its own throwaway Docker stack. The `tenanted` lane reruns each client
  suite with tenant scoping on and no tenant header, and any divergence from the flag-off lane
  fails the run either way. The recorded isolation run is 45 assertions, 0 failures.
