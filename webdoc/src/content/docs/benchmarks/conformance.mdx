---
title: Conformance
description: "The Queen 1.x conformance record: every client suite run against every applicable PostgreSQL topology in its own throwaway stack, the tenancy parity gate, and the two-tenant isolation run."
type: reference
sidebar:
  order: 8
sourceOfTruth:
...
<Aside type="caution">
This is the conformance record of **Queen 1.x**, whose storage was PostgreSQL: every topology
below is a PostgreSQL stack. 2.0 replaced that storage with a replicated log, so this matrix
describes the 1.x broker and not 2.0.
</Aside>
...
Running the client suites on `ha` is what made 1.x high availability a tested claim rather than
a topology diagram: the same unmodified suite, pointed at one broker of a mesh pair sharing one
PostgreSQL, had to produce the same result it produced against a single broker.
...
`note` lines rather than asserted: in 1.x PostgreSQL was the authority on those paths and every
stored-procedure call carried the tenant, so the cost was a re-query, not a leak.
...
- **The `mesh` suite asserted connection, not behaviour.** In 1.x, cross-replica wake latency,
  dead-peer detection and reconnection were checked by hand with a separate script. 2.0 has no
  mesh.
