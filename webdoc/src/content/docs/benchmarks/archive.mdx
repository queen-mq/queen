...
engine. The log engine is a greenfield replacement with no data migration: the shipping broker
embeds one schema, the log engine's, and applies it to an empty database at boot. No `seg_*`
object appears anywhere in it. So these numbers describe a schema the product no longer has.
...
| `2026-07-23-openloop/` | The session that produced this project's best headline number. Its stdout was not archived, so the configuration cannot be checked against the result. Its own configuration line records deduplication off, which is not a configuration this project publishes a headline from |
...
| --- | --- |
| Long-running stability, and sustained throughput | [Throughput 24h 1M](/benchmarks/soak-24h) |
