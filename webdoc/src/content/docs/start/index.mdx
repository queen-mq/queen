...
- **1 000 000 messages a second**, sustained for 24 hours. 86,369,975,300 messages, zero
  restarts, broker memory flat at 4.1 GB. [Throughput 24h 1M](/benchmarks/soak-24h).
...
That range is the whole bet, because most systems are built for one end of it: a design
that gives each entity a physical object pays for cardinality in structure, and one that
re-derives ordering from a scan on every read pays for throughput in standing rows. Queen
was measured against Kafka, RabbitMQ and pgmq on one machine with matched resources, and
the cost curves cross where those designs predict.
...
This site is written from the current source, and three rules hold everywhere on it:
