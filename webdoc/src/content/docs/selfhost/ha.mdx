...
| Hot-list reseed floor | `QUEEN_HOTLIST_RESEED_MS`, default 30000 ms | Rebuilds a candidate ring from PostgreSQL even when it is non-empty, recovering a partition dropped by a missed mark or a dropped hint. Bounded to the partitions written in the last `QUEEN_HOTLIST_RESEED_WINDOW_MS`, which covers every case a push can create, and so every dropped hint. |
| Hot-list full reseed | `QUEEN_HOTLIST_RESEED_FULL_MS`, default 300000 ms | The same walk over every partition of the queue. This is the one that recovers pendingness no write explains, such as a ring entry cleared in error or a claim stranded by a dropped pop. A backward seek does not wait for it: the broker that serves the seek walks in full immediately and tells its peers over the mesh. |
