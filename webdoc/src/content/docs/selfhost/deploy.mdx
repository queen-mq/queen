...
The image is `smartnessai/queen-mq:1.0.0`. Its runtime stage is `ubuntu:24.04`,
its working directory is `/app`, and its command is `./bin/queen-seg`.
...
3. **Give the disk spool a real home.** `FILE_BUFFER_DIR` defaults to
   ...
   ```bash
   docker run -d --name queen --network queen -p 6632:6632 -e PG_HOST=queen-pg -e PG_PASSWORD=postgres -v queen-spool:/var/lib/queen/buffers smartnessai/queen-mq:1.0.0
   ```
...
| `LOG_LEVEL` | `info` | Accepts full `EnvFilter` syntax, e.g. `info,queen_seg::pop=debug`. `RUST_LOG` overrides it. |
...
`build.sh <registry>` is the publish path: it reads `name` and `version` from
`server/server.json`, builds for `linux/amd64` with `QUEENCTL_VERSION` set to
that version, tags the result `<registry>/<name>:<version>`, and pushes it. The
version in `server/server.json` is therefore the single source of truth for the
image tag, for what `/health` reports, and for what `queenctl version` reports.
`server/build.rs` embeds it into the binary as `QUEEN_VERSION`.
...
The result is `server/target/release/queen-seg`. Two build-time couplings are
...
<Aside type="note">
The dashboard's assets are served, but the SPA boots by calling `GET /auth/me`,
which only `queen_proxy` serves. A broker-only deployment will not get past the
dashboard's gate. That is a known gap, not a misconfiguration on your side.
</Aside>
