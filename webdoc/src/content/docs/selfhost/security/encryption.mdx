...
The rest of this page covers both, and how to verify them: with only one of the
two in place the broker stores plaintext without raising an error.
...
`QUEEN_ENCRYPTION_KEY` is read at boot, and every rejection
...
response says `queued`, and the data path reports nothing further. The boot log
is where that shows up, which is why the checks below belong in the deploy.
...
Verify at the deployment level, since the queue's flag on its own does not tell
you whether the key loaded:
...
The drain is what makes that safe. One key is loaded per process and decryption
tries only that key, so every message still in the log that was written under the
old key becomes an undecryptable envelope from the instant the new key is in
place.
