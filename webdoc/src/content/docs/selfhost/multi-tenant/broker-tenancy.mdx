...
it into the SQL name-resolution functions — for several stored procedures, as a
...
- **Row deletions are scoped**, so a retention or delete path cannot reach another
  tenant's rows through a name match.
...
means every cluster shares the default tenant's namespace — the header is simply never
read. A broker with tenancy on behind a proxy with injection off means the same thing by
