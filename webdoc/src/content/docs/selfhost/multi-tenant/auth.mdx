...
3. Otherwise, auto-provision — **only** when `QUEEN_PROXY_AUTOPROVISION=true`. The new
...
label to route by. `x-queen-act-cluster` supplies the cluster explicitly — a slug or a
cluster UUID:
