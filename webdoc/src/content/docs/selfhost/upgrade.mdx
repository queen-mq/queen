...
database, and the DDL the broker applies at boot only *creates*. `CREATE TABLE`
declares the final shape, once, so a deployment carries no migration files, no
version table and no upgrade ALTERs. The whole page follows from that:

Upgrading a deployment means standing up the new version against a fresh
database and cutting traffic over. Pointing a
new build at a database created by an earlier version is unsupported: the boot
apply only creates, and leaves existing tables in the shape it found them.
...
If you cannot drain (a backlog you must not lose), copy it at the application
level: read the messages out through the old deployment's own API and push them
into the new one as new messages, giving each one a deterministic
`transactionId` so a retry of your copy loop cannot duplicate it.

### Rollback

Rollback is the same cutover in the other direction, so it is a matter of
retiring the old deployment late: the old deployment and its database still
exist until you do. Keep both running (producers stopped, consumers drained)
until you are confident in the new deployment; rolling back is pointing clients
back at the old broker. That route closes once you delete the old database:
messages pushed to the new deployment in the meantime would need the
same copy-loop treatment in reverse.

## Coming from 0.16

The previous Queen broker was a C++ implementation on a different storage engine.
Version 1.0 is not an update of it: moving to it is a cutover of the kind above,
the always-virgin model again rather than a special case, and it carries no data.

- The `queen migrate` subcommand reports exactly that. It logs that migration
