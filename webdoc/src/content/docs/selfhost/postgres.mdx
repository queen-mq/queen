...
**PostgreSQL 14 or newer.** The schema uses `gen_random_uuid()` as a column
default, which is built into PostgreSQL from 13 onwards, and per-table storage
parameters including `autovacuum_vacuum_insert_scale_factor` (13) and
`vacuum_truncate` (12). The repository's test harness and the benchmark rigs both
run `postgres:16`; the published broker image ships the PostgreSQL 18 client
tools. Anything from 14 up satisfies the DDL.
...
- **`GRANT` capability**, because `019_streams_schema.sql` issues grants (see
  below).
...
`server/sql/procedures/` in lexical order. That order is not cosmetic: the
log-engine files in the `04x` range redefine stored procedures that the earlier
files create, so applying them out of order leaves the wrong definitions in
place. `server/src/schema.rs` holds the authoritative list of 26 procedure files
...
<Aside type="caution">
`QUEEN_APPLY_SCHEMA=0` means **you** own keeping the schema current across
upgrades. The broker's normal contract is that a new version's DDL is applied
before that version serves a request; with the flag off, a version whose SQL
expects a column you have not added will fail at runtime rather than at boot. See
[Upgrades](/selfhost/upgrade).
</Aside>

### `queen_streams` is granted to PUBLIC

`019_streams_schema.sql` ends with three grants:
...
(`schema.sql` plus 26 procedure files, as 27 separate batches) and unlocks. The
...
Two schemas. `queen` holds the broker's own tables, of which six carry the log
engine (`log_queues`, `log_partitions`, `log_segments`, `log_txns`,
`log_consumers`, `log_dlq`), alongside queue configuration, traces, statistics and
metrics tables. `queen_streams` holds the two stream tables described above.
