...
- **TLS is terminated in front.** `rustls` is linked in, but only as a *client*:
  for PostgreSQL connections and for fetching a JWKS document. Nothing in the
  broker accepts an HTTPS connection. Termination is a reverse proxy's job, or
  `queen_proxy`'s.
- **Cross-origin browser calls are not served.** There is no `Access-Control-*`
  handling anywhere in the request path, so a browser cannot call the broker
  cross-origin.
- **Authentication is opt-in.** With `JWT_ENABLED` unset, every route is
...
- **Every replica serves every route.** There is no leader. Only background
  maintenance is leader-gated, by PostgreSQL advisory locks, and the election is
  per cycle.

## Four behaviours to know before you deploy

Each of these is explained in full on the page that owns it; they are collected
here because they shape an operational decision early.
...
- [Security](/selfhost/security/): trust boundaries, JWT algorithms and JWKS key
  discovery, broker-to-PostgreSQL TLS, and at-rest payload encryption.
- [Observability](/selfhost/observability/): the log's periodic `rates` and
  `sizes` blocks, the metrics endpoints, and why `/metrics/prometheus` is not a
  per-tenant surface.
...
- [Sizing](/selfhost/sizing/): capacity planning in commits rather than messages,
  and why batch size is the lever that moves the result.
- [Backup](/selfhost/backup/): `pg_dump`, because the database is the whole
  state.
- [Dashboard](/selfhost/dashboard/): what the bundled SPA shows.
- [Multi-tenant](/selfhost/multi-tenant/): building your own multi-tenant service
  on `queen_proxy`, with tenants, API keys, quotas, metering, isolation, and the
  broker's `QUEEN_TENANCY_HEADER`.

For what the broker actually exposes on the wire, see the
[route table](/reference/http/routes/); for every environment variable with the
default the code applies, see the [configuration reference](/reference/config/).
For the measured runs behind any performance claim, see
