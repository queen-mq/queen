...
<Aside type="caution">
The facade is a **preview**. It is not part of a Queen release, and what has been run end to end
against a real broker is narrower than what is implemented: boto3 and the `aws` CLI over the JSON
protocol, and boto3's SNS client over the Query one. The client matrix and the differential run
against real AWS are not done, so the divergences at the bottom of this page are the ones known
