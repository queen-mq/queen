...
sourceOfTruth:
  - queen-kafka/src/versions.rs
  - queen-kafka/src/handlers/metadata.rs
  - queen-kafka/src/handlers/produce.rs
  - queen-kafka/src/handlers/fetch.rs
  - queen-kafka/src/handlers/list_offsets.rs
  - queen-kafka/src/coordinator/mod.rs
  - queen-kafka/src/coordinator/group.rs
  - queen-kafka/src/offsets.rs
  - queen-kafka/src/records.rs
---
...
<Aside type="caution">
The facade is a **preview**. It is not part of a Queen release, its behaviour is verified by
its own compatibility suite rather than by a released client matrix, and the deviations at the
bottom of this page are the ones known today rather than a closed list.
</Aside>
...
envelope below. There is exactly one broker, node 0, and it is the facade itself: it advertises
itself as leader of every partition and answers `FindCoordinator` with its own address,
whatever the Queen cluster behind it looks like.

## The support matrix

Every Kafka client opens a connection with `ApiVersions` and then speaks only what the answer
allowed. That answer is built from one table in `queen-kafka/src/versions.rs`, which is also
...
**Transactions and exactly-once.** `InitProducerId`, `AddPartitionsToTxn`, `EndTxn` and
`TxnOffsetCommit` are not advertised, and `Produce` refuses a transactional id, a transactional
or control batch, and any batch carrying a producer id. The refusal is the point: a producer
that believed its sequence numbers were being enforced would get at-least-once with duplicates
on every retry and find out in production. A `transactional.id` producer is told
`TRANSACTIONAL_ID_AUTHORIZATION_FAILED`, which every client treats as fatal, with a message
naming the reason on Produce v8 and above. Kafka Streams needs exactly-once, so Kafka Streams
applications do not run against the facade.

**The idempotent producer.** Same refusal, same reason. Set `enable.idempotence=false`. What
you get instead is the at-least-once every non-idempotent Kafka producer already has.
...
Queen's, is time-based, and is off by default on a topic the facade auto-creates.
...
million keys, and there is no `DeleteGroups` yet to remove them.
...
They survive a facade restart, and they are not yet the broker's native consumer-group cursor,
so Kafka groups do not appear in the console's consumer-group lag views.
...
**Two credentials of one tenant are two groups.** The group registry is keyed by (credential,
group id), because `orders-consumer` is a name two strangers pick and a registry keyed by the
group id alone would put two tenants in one group. Queen scopes committed offsets by **tenant**
instead. The two agree while a tenant has one credential and disagree the moment it has two, a
key rotation or a per-service key: two consumers of one group id become the sole member of two
groups, each assigned everything by its own leader, both writing the same offset keys. The
facade cannot resolve this from the inside today, so it logs when one group id goes live under
a second credential. Give one consumer group one credential.
