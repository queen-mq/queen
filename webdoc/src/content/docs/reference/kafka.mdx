...
sourceOfTruth:
  ...
  - protocols/queen-kafka/src/handlers/end_txn.rs
...
`queen-kafka` runs inside the broker process when `QUEEN_KAFKA_EMBEDDED=true`. It speaks the
Kafka wire protocol to clients and calls the broker's own router in-process, meeting the same
routes, authentication, tenancy, push admission and forwarding to the leader that an HTTP client
meets. A Kafka produce is a Queen push and a Kafka fetch is Queen's `POST /api/v1/fetch`. An
unmodified Kafka producer or consumer reaches Queen by changing `bootstrap.servers`, and nothing
else. Running it is [the operator page](/deploy/kafka); this page is the contract a client gets.

<Aside type="caution">
The facade is part of the broker binary and runs in its process when `QUEEN_KAFKA_EMBEDDED=true`.
What is narrower than the broker's own guarantees is the verification behind this page: the behaviour
...
envelope below. In the default mode there is exactly one Kafka broker, node 0, and it is the
...
of the bundle, so the records and the offsets commit in one entry of the replicated log or neither
of them does. `EndTxn(abort)` drops the stage and writes nothing at all.
...
stated rather than hidden: a facade that has served a million throwaway group ids holds the
committed offsets of a million groups. `DeleteGroups` and `OffsetDelete` are both advertised and
both remove them irreversibly, so removal here is an administrator's action rather than a timer
nobody set.
...
**`acks=1` and `acks=-1` are the same path.** Every push is answered once its entry is committed
to the replicated log. The facade has no cheaper tier to answer `acks=1` from, so both are answered
...
followers; here replication is the raft log's business, not the facade's. What bounds the wait is
the facade's own ten-second budget for a call to Queen, well below every client's 30 second
`request.timeout.ms`.
...
`offsetsForTimes` returns null for. The alternative would be a scan or an approximate answer,
and an approximate answer is a consumer silently starting somewhere other than where it asked. The partition is still probed
...
**Committed offsets are Queen consumer-group positions by default.** With
`QUEEN_KAFKA_OFFSET_STORE=positions`, the default, a committed offset is the broker's own cursor
for (group, queue, partition), written through the transaction's `positions` rider and read back
through `POST /api/v1/consumer-groups/positions`. One commit is one entry in the replicated log
whatever its width, and a Kafka group is a Queen consumer group: the consumer-group views show its
lag beside every native group, and a native consumer of the same group name resumes where it
committed. The transactional fence and the group's index row stay in Queen's key/value store.

`QUEEN_KAFKA_OFFSET_STORE=kv` keeps the offsets in the key/value store instead, one key per
(group, topic, partition) under the `queen-kafka` namespace, spelled
`qk:group:<group>:<topic>:<partition>` with everything outside `[A-Za-z0-9._-]` percent-encoded. A
facade that reaches Queen over an explicit `QUEEN_URL` always uses this store. Those keys are not
shown in a 2.0 broker's consumer-group views. Offsets are not moved from one store to the other
when the setting changes. Either way they survive a facade restart.
...
Through the proxy it does. When the facade reaches Queen through the proxy (an explicit
`QUEEN_URL` naming it keeps the facade on HTTP to that address), the proxy resolves an api-key
bearer through its normal credential path, so a key rotation or a per-service key resolves to the
same tenant and the two credentials are ONE group over one set of committed offsets, which is what
a group of that name should be.
...
When Queen answers a call with `429` and a `Retry-After` (the broker's push admission, after a push
waited the whole hold for room, or a rate cap when the facade reaches Queen through the proxy), Kafka has a
field for exactly that, and the facade puts the same number in it on the three responses in the
hot path.
