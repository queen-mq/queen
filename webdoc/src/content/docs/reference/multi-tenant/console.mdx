...
1. **The `Host` header resolves to a cluster**, or the request is `421`
   `{"code":"cluster_unknown"}`. The console resolves the cluster from `Host` alone: unlike
   the data plane it does not consult `x-queen-act-cluster`, so one browser session on one
   hostname acts on exactly one cluster.
...
| `421` | `cluster_unknown` | The `Host` header resolves to no cluster |
