...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): main.rs, db.rs and gateway.rs gained the replay and KV console work; the tenancy header, its propagation and the broker's own scoping are unchanged.
---
...
entirely: `GET /api/v1/status`, the system and PostgreSQL analytics, and the Prometheus
exposition (whose per-queue series sum across tenants). The proxy classifies each of
them as operator or blocked for exactly that reason. The streams routes used to be on
this list and no longer are: `/streams/v1/*` resolves the header like every other
tenant surface ([Isolation](/reference/multi-tenant/isolation/#streams-are-tenant-scoped)).
