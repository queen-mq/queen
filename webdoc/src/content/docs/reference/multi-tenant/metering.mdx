...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): gateway.rs grew a third
# source of `push` messages, a replay that moved one, booked on a second sample with reqs = 0;
# the row-id replay's request is now buffered for its destination override and the address-keyed
# one is not, which the request-bytes paragraph now says. Both are written down above; nothing
# else in meter.rs, spool.rs or the migrations moved.
---
...
cycle whose body says `success: false` (the transaction rolled back, sink writes
included), and a response the proxy cannot parse (`msgs = 0` with a warning).
