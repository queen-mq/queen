...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): the sentence saying a viewer writes nothing is now true, and says so: classify's messages arm was DELETE-only, so the retry POST inherited read. Both replay routes are queue admin.
---
