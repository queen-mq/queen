...
[Usage metering](/reference/multi-tenant/metering/); it simply has no value to compare
against out of the box.

`features.streams` opens the routes but is not the whole grant: the broker refuses a
non-default tenant's **new** query registrations until the cell database holds an
enabled `queen_streams.quota` row for that cluster's `broker_tenant_uuid`, with an
optional `max_queries` cap (`NULL` is unlimited). Provisioning a streams-enabled
cluster therefore means one insert on the **cell** database, not on `queen_proxy`:

```sql
INSERT INTO queen_streams.quota (tenant_id, max_queries)
VALUES ('<broker_tenant_uuid>', 100);
```

Absence is a denial by design, the same posture as the KV grant, because the tenant
header is opaque and a fail-open default would hand query space to any invented id.
...
[Deploying queen_proxy](/deploy/multi-tenant/), which leaves
...
[Deploying queen_proxy](/deploy/multi-tenant/) covers what
...
[self-serve console](/reference/multi-tenant/console/) is the caller that uses it
...
- [The self-serve console](/reference/multi-tenant/console/): the caller that drives
  these functions today, and what it writes to the audit trail.
- [Credentials and authorization](/reference/multi-tenant/auth/): what a key or a
  session can actually do with these rows.
- [Quotas and rate limits](/reference/multi-tenant/quotas/): how each plan column is
