...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): queue configure now merges and sends only the flags that were typed (so --dlq=false disables dead-lettering), apply -f sends mode replace, refuses a manifest key it cannot bind, and now puts every key the manifest writes on the wire with the value it wrote (an explicit false and an explicit zero included), and dlq retry runs on the move primitive.
---
...
partitions, consumer groups, DLQ, traces, maintenance, metrics, analytics). Version 1.3.0, aligned
...
since 1.6.0 **moves** the newest dead-letter snapshot at that address: claim the row under a lock,
push the frame to its own queue and partition, delete the row, one transaction.
...
and removes **only that group's record**. The other groups' records survive. That is the difference
...
`null` is accepted too and asks for that option's default, which is what leaving the key out
already means.
</Aside>
