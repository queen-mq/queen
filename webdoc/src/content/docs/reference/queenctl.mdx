...
replay.** Replay itself is a broker route,
...
<Aside type="danger">
No `queenctl` command wraps that retry route, so `dlq drain` is deletion and nothing else, and a
drained row is gone. To replay dead-lettered messages, call the route directly, or read the rows
with `dlq list` or `messages get` and re-publish them with `queenctl push`.
</Aside>
