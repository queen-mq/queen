...
Since 1.6.0 that route runs on a **move**: one transaction claims the newest
dead-letter row at the address, pushes it back into the log and deletes the row,
and the replayed message carries the deterministic transaction id
`dlq:<dead-letter row id>`. Two consequences for a caller. A second call for the
same address answers `404`, because the row is gone, so a retry after a lost
