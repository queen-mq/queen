...
| --- | --- | --- |
| `maxAttempts` | `10` for ordinary requests; unbounded for a long-poll pop | Total attempts including the first. Setting it explicitly applies the same bound to both kinds. |
| `baseMs` | `500` | First delay when the response carries no `Retry-After`. Doubles per attempt. |
...
- **`/configure` is a full replace.** Keys absent from the body reset to the broker's own
  defaults, not to whatever the queue had before. Send the complete intended configuration every
  time. The full option list lives in [Queue options](/reference/queue-options/).
...
<Aside type="danger">
`autoAck` means two different things depending on the method. On `pop()` it is sent to the
broker as `autoAck=true`, which commits the cursor inside the pop transaction: at-most-once
delivery, the message is gone whether or not you process it. On `consume()` it is never sent to
the broker: the client acks after the handler resolves and nacks when it throws. The
`ConsumerManager` explicitly does not forward the parameter.
</Aside>
...
<Aside type="note">
A transaction is atomic, not exactly-once end to end. A lost response plus a client retry still
duplicates unless the `transactionId` is deterministic and inside the dedup window.
</Aside>
