...
The listing is read-only, but `Admin.RetryMessage(ctx, partitionID, transactionID)` replays: the
broker re-pushes the dead-letter payload snapshot to its own queue and partition and drops the DLQ
row once that push is accepted. Only dead-lettered addresses replay, and a live one answers `404`.
It is **not idempotent**, so the client sends it with failover retry disabled and a caller that
sees a failure has to re-read the DLQ before trying again rather than repeating the call.
`Admin.MoveMessageToDLQ` is still a stub, because the broker registers no force-move route.
