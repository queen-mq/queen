...
sourceOfTruth:
  ...
  - clients/client-php/src/Queen.php
  - clients/client-php/src/Admin.php
  - clients/client-php/src/Support/Defaults.php
  - clients/client-php/src/Support/Uuid.php
  - clients/client-php/src/Builders/QueueBuilder.php
  - clients/client-php/src/Builders/PushBuilder.php
  - clients/client-php/src/Builders/ConsumeBuilder.php
  - clients/client-php/src/Builders/TransactionBuilder.php
  - clients/client-php/src/Builders/DLQBuilder.php
  - clients/client-php/src/Builders/OperationBuilder.php
  - clients/client-php/src/Consumer/HighLevelConsumer.php
  - clients/client-php/src/Http/Retry429Policy.php
  - clients/client-php/src/Exceptions/HttpException.php
  - clients/client-php/src/Exceptions/ErrorCode.php
  - clients/client-php/src/Laravel/QueenServiceProvider.php
  - clients/client-php/src/Laravel/QueenFacade.php
  - clients/client-php/src/Laravel/Queue/QueenConnector.php
  - clients/client-php/src/Laravel/Queue/QueenQueue.php
  - clients/client-php/src/Laravel/Queue/QueenJob.php
  - clients/client-php/src/Laravel/Queue/ProcessLeaseRenewer.php
  - clients/client-php/src/Laravel/Queue/SyncedFailedJobProvider.php
  - clients/client-php/src/Laravel/Contracts/QueenPartitionable.php
  - clients/client-php/src/Laravel/Commands/ConsumeCommand.php
  ...
  - clients/client-php/src/Laravel/Supervisor/WorkerTelemetry.php
...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): QUEUE_DEFAULTS is nine keys, the configure paragraph is rewritten for the merge rule, and retryMessage carries the move primitive's consequences.
---

The Composer package is `queen-mq/php-client`, PSR-4 namespace `Queen\`. It requires PHP 8.3 or
...
package and is optional. The source is prepared for the aligned release train, but Packagist has
no tagged release yet; install it from the repository only if you intentionally accept that status.
...
`Defaults::QUEUE_DEFAULTS` carries nine keys: `leaseTime`, `retryLimit`, `priority`,
`delayedProcessing`, `windowBuffer`, `maxSize`, `retentionSeconds`, `completedRetentionSeconds`
and `encryptionEnabled`, with the values listed under
...
Since 1.6.0 `/configure` [merges](/reference/queue-options/): an option the body does not carry
keeps the value the queue already has, and only `"mode": "replace"` resets what the body omits.
This client sends no `mode`, so `create()` always merges. Because `config()` merges over
`QUEUE_DEFAULTS`, those nine keys are on the wire whether you named them or not and land on the
client's default rather than on the queue's stored value; the twelve options outside that array,
`dedupWindowSeconds` and `retentionEnabled` among them, are the ones a partial call now leaves
alone.
...
`execute(): mixed`. `execute()` returns the broker's per-item array for a direct push. A 2.0
broker answers each item `queued`, `duplicate` or `error`. Per item, `onError` fires (and
`execute()` throws when it is absent) only for `failed`, a status the 2.0 broker never returns:
an `error` item reaches no callback and throws nothing, so check each item's `status` in the
returned array. A buffered push returns without sending once the buffer accepts the message. At
`maxSize` waiting messages the add flushes inline and keeps retrying under a `maxWaitMillis`
...
Since broker 1.6.0 that route runs on a **move**: one transaction pushes a dead-letter row at the
address back into the log and deletes the row (on a 2.0 broker, one entry in the replicated log),
and the replayed message carries the deterministic transaction id `dlq:<dead-letter row id>`. A
second call for the same address answers `404` rather than replaying a second copy, only the
addressed consumer group's record is removed, and a `result: "duplicate"` verdict means nothing
was written and nothing was removed. See [Messages and dead letters](/reference/http/messages-dlq).

## `TransactionBuilder`

`$queen->transaction()`. Pushes and acks in one transaction: one entry in the replicated log,
all-or-nothing.
...
A 2.0 broker answers `refreshConsumerStats()` with a 200 and `"refreshed": false`: its counters
are updated as log entries apply, so there is nothing to refresh.
...
in `test/run.sh` executes it. The rows above are read off `clients/client-php/src`, not off a
