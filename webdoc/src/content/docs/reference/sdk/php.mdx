...
boundary. Short, idempotent workloads can opt into a larger prefetch and set
`ack_batch` to the same value: one pop then feeds the standard sequential
Laravel worker, and the final delete commits the complete lease through
`/ack/batch`. This is still at-least-once delivery, but it deliberately widens
the crash duplicate window and unprocessed prefetched jobs can remain leased
until `retry_after`. The lease must cover the worst-case runtime of the entire
prefetched batch. Leave both at `1` for long jobs, strict per-job confirmation,
or comma-separated priority queues. A reentrant `pop()` before the active
prefetched job is deleted or released fails explicitly.
