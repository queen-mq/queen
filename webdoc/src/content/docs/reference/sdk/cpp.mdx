...
| --- | --- | --- |
| `config(const QueueConfig&)` | — | `QueueBuilder&` |
...
| `concurrency(int)` | — | `1` | `consume()` only. Thread-pool workers. |
| `limit(int)` | — | `0` (unlimited) | `consume()` only. |
| `idle_millis(int)` | — | `0` (no timeout) | `consume()` only. |
| `renew_lease(bool, int interval_millis = 60000)` | — | `false` | `consume()` only. See the note below. |
| `each()` | — | batch mode | `consume()` only. One message per handler call. |
...
`json pop()` — the `messages` array, or an empty array. Route selection: queue plus non-`Default`
...
All four return `json` — `nullptr` for an empty body or a `204` (the broker sends no body at all
on `204`) — and throw `HttpError` on any status `>= 400`. `request_timeout_millis` of `0` uses the
