...
`retry_message` is a dead-letter replay. Since broker 1.6.0 it MOVES the newest dead-letter row
at the address: one transaction claims the row, pushes the snapshot back into the log and deletes
it, and the replayed frame carries the derived transaction id `dlq:<row id>`, so a second call
answers `404` rather than appending a second copy. Only the addressed consumer group's record is
removed. It errors on a message that is not in the DLQ.
