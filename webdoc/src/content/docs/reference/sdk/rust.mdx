...
`pop` returns `Err` on failure rather than an empty vector. An empty claim returns `Ok` with
nothing in it.
...
`QueueOptions` is all `Option` fields and skips every `None`, so a `configure` sends only what
you set. Since 1.6.0 `/configure` [merges](/reference/queue-options/), which means such a call
now edits what it names and keeps the rest, where it used to reset the other twenty options on
its way past. When the request **is** the whole configuration, `ConfigureRequest::replace(true)`
puts `"mode": "replace"` on the wire and every option the request does not carry goes back to
its default; leaving it alone sends no `mode` key at all, so the request stays byte-identical to
the one this crate has always produced, and an older broker replaces either way.
...
| `ack_with(&Message, AckStatus)` | `Retry` and `Dlq` reach the broker as their own statuses rather than collapsing to a boolean. |
...
`retry_message` is a dead-letter replay. Since broker 1.6.0 it MOVES a dead-letter row at the
address: one transaction pushes the row's snapshot back into the log and deletes the row (on a
2.0 broker, one entry in the replicated log), and the replayed frame carries the derived
transaction id `dlq:<row id>`, so a second call answers `404` rather than appending a second
copy. Only the addressed consumer group's record is removed. It errors on a message that is not
in the DLQ.
...
`system_metrics(params)`, `worker_metrics(params)`, `health()`,
`metrics() -> String`.
