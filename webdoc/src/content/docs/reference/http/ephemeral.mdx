...
verifiedBy:
  ...
  - server/src/tests_unit/ephemeral_engine.rs
---

Eight routes over one storage class whose contents live in a broker node's memory and nowhere
else. [Ephemeral queues](/use/ephemeral) is the contract these routes serve; this page is the wire.

<Aside type="note">
Every 2.0 broker serves these routes, and no variable turns them off. The depth read answers `404`
for a queue that does not exist; no other route here answers `404` for a missing queue.
</Aside>
...
## Which node holds a queue

The contents of an ephemeral queue are not replicated: they live in the memory of the node that
serves the queue's requests. On a single node that is the node. In a cluster a follower forwards
every ephemeral request to the leader, as it does every other `/api/` request, so the queues live
in the leader's memory. A leader change moves the surface to the new leader: what the old leader
held is not there, which is the loss the class declares, and an ack for an id the old leader minted
answers `stale`.

Declarations are different: `configure` and the queue delete write to the replicated state, so a
declared queue survives a restart as configuration, never as contents. [configure](#configure) says
when a node applies one.
...
`201` with `{"pushed": 1}`. One queue per request, unlike the durable push whose items each name
...
| `messages` | yes | at most 10,000 per call. Only each element's `payload` is read |
...
it and nothing to re-query, so an idle waiter costs a parked task and nothing else.

**The `id` is opaque.** It encodes the incarnation of the broker process that minted it, which is
what lets an ack arriving after a restart or a leader change answer `stale` instead of settling
somebody else's message. Do not parse it, and do not store it as a durable reference to anything.
...
dead-letter queue and the trace store, and this class has neither.
...
| `stale` | the id was minted by a broker process that is gone: a restart, or a leader change in a cluster. That is the loss contract, not a fault |
...
`201` with `{"queue": "presence", "options": {…}, "declared": true}`, the options echoed as sent.
The declaration is one entry in the replicated log. Once it has committed, the node that answered
the call applies it to its memory, and every node loads the stored declarations when it starts, so a
declared queue comes back configured, and empty, after a restart. Another node applies a
declaration committed after it started only when it next starts.
...
A 2.0 broker does not validate `options` on this route: `queue` must be present and not empty, and
the object is stored as sent. When it is applied, a key outside the table above is ignored, and so
is a value of the wrong type (a `ttlSeconds` of `1.5`, a `policy` that is neither `reject` nor
`dropOldest`): that option keeps its default. Check the spelling of every key, because a typo is not
reported.
...
`{"queue": "presence", "deleted": true, "declared": true}`. It removes the declaration from the
replicated state, and the node that answered drops the contents and the cursors from its memory.
`deleted` and `declared` both say whether a declaration was there to remove; the contents are
dropped either way.
...
Both read the answering node's memory and nothing else, which is what makes them safe to poll every
second, and safe to poll during the incident they exist for. In a cluster that node is the leader,
the node every ephemeral request reaches.
...
handlers are failing. `cellBytes` is the node's whole ephemeral footprint, not the tenant's,
because it is the number a node-wide `503` is measured against.
...
| `400` | `ephemeral_bad_request` | shape: no queue, a name over 512 bytes or carrying a control character, more than 10,000 items in one call, a body that is not the expected JSON |
| `400` | `bad_request`, `bad_body` | `configure` without a `queue`, or a `configure` body that is not JSON |
...
| `500` | `internal` | `configure` or `delete` failed inside the broker |
| `503` | `ephemeral_unavailable` | the node is at `QUEEN_EPHEMERAL_MAX_BYTES`, or its per-tenant table is full |
| `503` | `ephemeral_disabled` | an operator paused the surface, with `Retry-After`. See [the kill switch](/reference/http/system) |
| `503` | `no_leader`, `retry`, `timeout` | a follower could not reach the leader, or `configure` or `delete` could not commit, with `Retry-After: 1` |
...
`configure` and the queue delete are answered by the state machine rather than by the node's
ephemeral engine: the kill switch and the grant do not apply to them, and their failures are the
broker's common ones. The other six routes check the refusals outermost first, so the answer names
the reason an operator would act on: the kill switch before the grant, the grant before the rate,
the rate before the queue's own bounds. A paused surface must not leak, through a quota message,
that a tenant exists.

## Through the multi-tenant gateway

Two independent gates stand in front of a tenant that comes through the proxy, and **both** must
open: the plan's `ephemeral` feature at the proxy, which is off unless the plan says otherwise, and
the broker's own grant (a quota of kind `ephemeral`), whose absence refuses a tenant when
`QUEEN_EPHEMERAL_REQUIRE_GRANT` is on (its default follows `QUEEN_TENANCY_HEADER`). A self-hosted
broker with tenancy off has neither gate closed and serves the class from its first start. Both
refusals answer `403 feature_gated`.
