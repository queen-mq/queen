...
description: "Browsing messages, reading, deleting or replaying one by (partitionId, transactionId), and listing or purging the DLQ."
type: reference
tier: use
sidebar:
  order: 11
...
The JavaScript, Go, Rust and Laravel admin clients wrap this route as `retryMessage`,
and the Python one as `retry_message`. `queenctl dlq retry <partitionId> <transactionId>`
wraps it too, and requires `--yes` because the route is not idempotent. One surface
still predates it: the dashboard's dead-letter view offers purge only, so replay from
there means calling the route over HTTP yourself.

Because a resent request replays the message a second time, the Go client sends this
one with `WithoutFailoverRetry()`, meaning exactly one attempt against one backend,
rather than the default 5xx retry budget every other admin call uses.
