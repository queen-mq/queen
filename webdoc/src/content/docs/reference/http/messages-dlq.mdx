...
On a deployment that also holds legacy rows-engine queues, the log entries are
appended after the rows page and capped at `limit` independently, so a combined
page can hold up to twice `limit`.
