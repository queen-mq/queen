...
absent, it is the default tenant. The header is unauthenticated by design — the trust
boundary for it is the proxy — which is another reason this path must not be
...
handshake is replayable — and after the handshake, frames are unauthenticated JSON.
