...
sourceOfTruth:
  ...
  - server/src/auth.rs
  - server/src/db.rs
  - server/sql/procedures/011_traces.sql
  - server/sql/procedures/047_log_admin.sql
verifiedBy:
  - queen_proxy/scripts/isolation-smoke.sh
---
...
<Aside type="caution">
Nothing checks that the message exists. On the log engine a message's id lives
inside a compressed segment blob and is invisible to SQL, so `record_trace_v1`
resolves `message_id` from the rows-engine table, finds nothing, and inserts the
trace with `message_id = NULL`. A trace posted for a transaction id that was never
pushed is stored and returned by the read routes exactly like a real one. Traces
are evidence you produce, not evidence the broker corroborates.
</Aside>
...
resolved and the rows-engine payload joined in where there is one.
...
`message_payload` is `null` for log-engine messages: it comes from the rows-engine
`queen.messages` join, which log messages have no row in. Use
...
resolved per trace through its partition (looked up in both the log and rows
partition tables), and only traces belonging to the caller's tenant are counted
