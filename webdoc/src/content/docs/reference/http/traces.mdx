...
- **Nothing purges traces.** Retention, the DLQ purge and the metrics cleanup all
  ...
  trace at message rate. The one exception is a full tenant wipe: the
  operator-invoked `queen.delete_tenant_data_v1` deletes the wiped tenant's traces
  queue by queue, because traces are payload-derived and may not outlive the tenant.
