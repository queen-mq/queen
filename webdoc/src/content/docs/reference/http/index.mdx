...
# sourceOfTruth re-read 2026-09-09 (through ff644495): the three analytics routes 1.5.3 adds are already in the generated table; the conventions are unchanged.
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): auth.rs gained one explicit read-only arm, POST /api/v1/resources/kv/list, because a read whose cursor is a key has to travel in a body; the level matrix and the write-only set are unchanged.
---
...
[ephemeral](/reference/http/ephemeral) routes are the exception, and they are the exception on
