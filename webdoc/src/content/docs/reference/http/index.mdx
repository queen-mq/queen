...
terminates TLS: the multi-tenant gateway in `queen_proxy/`, or your own reverse proxy. Do not
...
| --- | --- | --- |
| `public` | anyone, with or without a token | — |
...
`/metrics`, `/metrics/prometheus`, `/`, `/assets/*` and `/favicon*`. `/metrics/prometheus` being
public is deliberate for scraping and is a hazard in a multi-tenant deployment, where per-queue
series sum across tenants; see [Self-hosting](/selfhost).
