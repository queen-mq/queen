...
sourceOfTruth:
  ...
  - queen_proxy/src/console.rs
  - queen_proxy/src/oauth.rs
---
...
| --- | --- |
| Paths and methods | the `Router::new()` chain: the broker's in `server/src/main.rs`, the proxy's in `queen_proxy/src/main.rs` plus the nested console and auth routers |
...
| `x-queen-query-params` | `typed`, `adhoc`, `forwarded` or `none`: how the query parameters were recovered |
...
route-class table in [Self-hosting](/selfhost).
</Aside>
