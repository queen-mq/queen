...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): the queue-option defaults paragraph is rewritten for the merge rule; every default value is unchanged.
---
...
created implicitly by a push. Since 1.6.0 `/configure` **merges**: on an edit an
omitted key keeps the value the queue already has, and the value in this table
is what a create lands on, what an explicit `null` restores, and what
