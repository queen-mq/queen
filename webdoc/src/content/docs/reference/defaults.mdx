...
2. **Queue configuration** — a column on the queue's configuration rows, written
...
| --- | --- | --- |
| Lease time | **60 s** on a queue created implicitly by a push; **300 s** after an explicit `/configure` | Two different rows. `queen.log_queues.lease_time` defaults to 60; `/configure` computes `leaseTime` with a default of 300 and writes it into that same column. So the value depends on how the queue came into existence. |
| Deduplication window | **3600 s, on** | Deduplication is not opt-in. `queen.log_queues.dedup_window_seconds` defaults to 3600 and `/configure` re-applies 3600 whenever `dedupWindowSeconds` is omitted. Only an explicit `0` disables it. |
...
| `dedupWindowSeconds` | `3600` | Segments-only, stored on the engine's queue row. Not echoed in the `/configure` response. |
...
| `60` s | The lease floor used when the queue has no engine row yet, or the lookup could not reach the database. |
