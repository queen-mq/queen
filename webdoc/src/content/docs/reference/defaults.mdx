...
| `QUEEN_HOTLIST_RESEED_FULL_MS` | `300000` | How often a ring is re-derived from *every* partition of its queue instead of only the recently written ones. It bounds how long a partition wrongly missing from a ring can stay missing. `0` makes every reseed a full walk. |
