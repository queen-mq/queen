...
## Three families you cannot alert on
...
`queen_dlq_depth` and `queen_dlq_depth_by_queue` are emitted, but they count
rows in `queen.dead_letter_queue`, the table the retired rows engine wrote. The
log engine dead-letters into `queen.log_dlq`, so both gauges read 0 on a healthy
current deployment even while `GET /api/v1/dlq` returns rows (that endpoint reads
both tables). For a working dead-letter signal use
`queen_cluster_dlq_total{scope="cluster"}`, which accumulates the broker's own
count of dead-letter transitions, or the DLQ listing endpoint.
