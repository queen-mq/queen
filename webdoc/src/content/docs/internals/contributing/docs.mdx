...
description: "How this site is built: the content tree, the strict frontmatter contract, the seven generators that derive fact tables, OpenAPI documents and benchmark figures from the source, and the drift guards that fail the build when a mirrored classifier changes."
type: how-to
tier: internals
sidebar:
  order: 16
...
| `gen-proxy-routes.mjs` | `generated/proxy-route-classes` | the request classifier in `queen_proxy/src/routes.rs`, applied to the broker's real route list |
...
`queen_proxy/src/routes.rs` classifies every broker-bound request into exactly one class, with
...
| `classify` in `queen_proxy/src/routes.rs` | `gen-proxy-routes.mjs` |
| `is_operator_route` in `queen_proxy/src/routes.rs` | `gen-proxy-routes.mjs` |
...
<Aside type="note">
`gen-snippets.mjs` also scans `clients/client-cpp`, which the workflow's path filter does not
include. No C++ file carries a marker today, so nothing is currently missed. But a snippet
added there would not be drift-checked until the filter is extended.
</Aside>
