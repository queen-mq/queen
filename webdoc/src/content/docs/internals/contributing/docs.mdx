...
| `gen-kafka-apis.mjs` | `generated/kafka-support-matrix` | the `ADVERTISED` table in `queen-kafka/src/versions.rs`, which is both the ApiVersions answer and the gate every request is checked against |
...
`queen-kafka/src/versions.rs` holds one `const ADVERTISED` table, and the Kafka facade both
...
| `ADVERTISED` in `queen-kafka/src/versions.rs` | `gen-kafka-apis.mjs` |
