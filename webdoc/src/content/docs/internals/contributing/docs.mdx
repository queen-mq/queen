...
description: "How this site is built: the content tree, the strict frontmatter contract, the seven generators that derive fact tables from the source, and the drift guards that fail the build."
type: how-to
tier: internals
sidebar:
  order: 19
...
which is the worst possible outcome. So each mirrored function is fingerprinted.
...
| `pnpm gen` | run all seven generators, writing partials, the OpenAPI documents and the figures |
