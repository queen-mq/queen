...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): main.rs registers three new routes and no new mesh frame; nothing this page describes moved.
---
...
1. **Waking a parked pop.** A write lands frames on replica A; a long-poll parked on replica B
   should re-poll now rather than at the end of its backoff interval.
...
`QUEEN_MESH_BIND_ADDR` narrows where the mesh listener binds without moving the HTTP listener.
`QUEEN_MESH_ADVERTISE_HOST` is the host this broker tells its peers to reach its HTTP surface on
(the port is always the HTTP listener's own `PORT`); it feeds the `http_addr` the handshake below
...
| 20 | `EPH_ADMIN` | `{op, tenant, queue}` | apply an [ephemeral-queue](/reference/http/ephemeral/) admin verb (`reset`, `delete`, `config_set`) to the local RAM-class state. The one frame family whose tenant is mandatory: these ops destroy, so a frame without a valid tenant is dropped rather than fanned out |
...
`MESSAGE_AVAILABLE` is a parked-pop wake, sent by every commit that lands frames in the log: a
push, a streams sink emit, a fired timer, a spool replay after an outage. `HOTLIST_DIRTY_BATCH`
marks a peer's candidate ring, and is sent only on a coalesced dirty transition: a partition pushed
a thousand times in one flush window produces one hint. A dropped hint for a landing write is healed by
...
with `{server_id, nonce, mac, http_addr, eph_epoch}` where `mac = HMAC-SHA256(secret, nonce)` over
the nonce's hex bytes. The listener verifies it in constant time before accepting any further
frame. An empty `QUEEN_SYNC_SECRET` means open mode: the `HELLO` is still exchanged, to learn the
peer's server id, but not verified.

The last two fields are the v2 additions and ride the existing frame rather than a new tag: the
broker's advertised HTTP address (`QUEEN_MESH_ADVERTISE_HOST` plus the HTTP `PORT`) and its
ephemeral-store epoch, which together admit the peer to the
[ephemeral queues'](/reference/http/ephemeral/) rendezvous ring. A `HELLO` without the pair, an
older build or half a pair, still authenticates exactly as before; the peer is only marked not
ephemeral-capable, so during a rolling upgrade the stats' `eph_members` count runs below
`servers_alive` and that is the expected reading, not an error.
...
- **The frame set includes `MAINTENANCE_MODE_SET`.** So an attacker who can reach the port and replay
  ...
  [the disk spool](/deploy/ha/). `POP_MAINTENANCE_MODE_SET` will likewise pause
  consumption cluster-wide.
- **The frame set includes `EPH_ADMIN`.** [Ephemeral queues](/reference/http/ephemeral/) live in
  broker RAM with no PostgreSQL underneath, so for them the same attacker is not limited to
  availability: `reset` and `delete` destroy their contents on receipt.
...
[Kubernetes network policy](/deploy/kubernetes/). Do not treat `QUEEN_SYNC_SECRET` as a substitute
for that. Set it as well, but the network boundary is what actually protects the cluster.
</Aside>

Nothing on the mesh can corrupt or misdeliver a durable queue's messages, because it carries no
data-plane writes and PostgreSQL rejects nothing it says. For the durable engine the exposure is
availability and operational control, not integrity; the RAM-class exception above is the one place
the frame set reaches state PostgreSQL does not hold.
...
- **It has no C++ wire compatibility requirement.** Rust-only clusters, which is why the wire is
  this simple. The one compatibility rule it does keep is cross-version tolerance during a rolling
  deploy: an unknown tag is skipped without closing the connection, and a new capability rides new
  fields on an existing frame (the tenant, the group, the `HELLO` v2 pair) whenever it can.
...
many peers are alive versus dead by the inbound-liveness threshold, how many of the live peers are
in the ephemeral rendezvous ring (`eph_members`), frames sent, received and
