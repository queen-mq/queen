...
Nothing in the retention cycle purges this table.
...
| `queen.retention_history` | one audit row per retention step that deleted something |
...
retention steps take exactly one partition lock each, so they cannot deadlock against a
