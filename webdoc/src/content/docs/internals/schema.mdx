...
file plus 24 files under `server/sql/procedures/` are embedded in the binary with `include_str!` and
...
The files numbered `040`–`048` are the log engine, and `049` drops what the rows engine left behind.
The surviving files in the `007`–`022` range are the shared management plane: configure, consumer
groups, stats readers, metrics, traces, streams. A subset of their procedures is superseded by later
redefinitions, which is why they are still applied. The rows engine's own message plane (`001`–`006`,
`016`, `021`) was deleted. See [Retired objects](/internals/legacy).
...
dependant. The rows *tables* are deliberately left alone.
