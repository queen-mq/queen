...
| `log_hotlist_reseed_window_v1(p_queue TEXT, p_group TEXT, p_after_write TIMESTAMPTZ, p_after_id UUID, p_limit INTEGER, p_window_ms BIGINT, p_tenant UUID DEFAULT …) RETURNS TABLE(r_id UUID, r_name TEXT, r_write TIMESTAMPTZ)` | the same enumeration bounded to partitions written in the last `p_window_ms`, which is what the reseed floor runs between full walks. Keyset on `(last_write_at, id)` so the access path stays on `idx_log_partitions_queue_write` even under a generic plan; pass `('-infinity', nil UUID)` to start a walk |
