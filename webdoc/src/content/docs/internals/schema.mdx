...
file plus exactly 27 files under `server/sql/procedures/` (`001`–`027`) are embedded in the binary
with `include_str!` and applied at every boot in lexical order under session advisory lock
`778120010`. All of it is idempotent (`CREATE OR REPLACE`, `IF NOT EXISTS`,
...
unsupported: a new version means a fresh database. See [Retired objects](/internals/legacy) for the history.
...
slow phases, and `027` the read-only quota poll every broker runs whether or not it sweeps. The
