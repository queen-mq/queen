...
both `001` (the table) and `019` (the trigger functions) is the point of it. The ordering is not
cosmetic anywhere: SQL-language function bodies resolve tables at creation time, so the DDL files
must land before every procedure body that names them.
