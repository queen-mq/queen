...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): 011_log_stats changed only get_queue_v2, which this page does not describe; db.rs and schema.sql likewise. Nothing here moved.
---
...
| Retained-bytes lane | `RETAINED_BYTES_INTERVAL_MS`, 600000 ms | `queen.stats.retained_bytes` and `segment_count` | how many payload bytes each queue retains (for the storage quota) and how many live segment rows (for the queue list) |
