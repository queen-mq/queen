...
A seek does not wait for either cadence. It runs a full walk on the spot and hands the partitions it
finds to the mesh, so the other brokers re-add them from the ordinary dirty hint rather than from
their own next full walk.
