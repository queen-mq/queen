...
| `leased` | zero frames because another worker holds a live lease until `T` | park in the wheel at `T + 300 ms` — **never** cleared |
...
Two mechanisms bound it. First, the serve path does not create a ring for a queue that does not
exist: ring registration happens *after* the group-seeded check, which reads the committed
`consumer_groups_metadata` marker, so a pop for a nonexistent queue allocates nothing. Second, the
idle sweep drops a queue's entire state when three conditions hold simultaneously, all checked
...
counters — followed by one `ring` line per busiest non-empty ring, ranked by `ready + wheel` and
