...
ESM only, Node 24 or newer, with the broker client and the streaming SDK in one package.
...
`duplicate` or `error`. Pass your own `transactionId` and the push becomes idempotent inside the
dedup window.
...
`success` off each item. To ack the input and push the output in one atomic commit, use
