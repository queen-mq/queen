...
The practical shape for a stage that must not double-publish is: derive the output `transactionId` deterministically from the input message id, and treat a `QDUP` failure on retry as "already done" rather than as an error. See [Idempotent producers](/use/examples/dedup) for how to choose that id and how long the window protects you.
