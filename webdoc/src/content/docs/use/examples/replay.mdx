...
description: "Reprocess what a queue already delivered: seek an existing group's cursor back to a timestamp, or subscribe a new group from the beginning, and what segment granularity costs at the boundary."
type: example
tier: use
sidebar:
  order: 24
...
`all` seeds the cursor just below the oldest **retained** offset, so a fresh group reads every message still on disk. It is what you must ask for explicitly: a grouped pop that sends no mode gets `new` (the broker default, `DEFAULT_SUBSCRIPTION_MODE`), which seeds at the partition's current end and skips the backlog entirely. `subscriptionFrom('now')` is equivalent to `new`, and `subscriptionFrom('<ISO timestamp>')` seeds at that point in history. A pop with no consumer group is always `all` and ignores the mode.
