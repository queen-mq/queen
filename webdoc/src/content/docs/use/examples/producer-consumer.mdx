...
`status` is `queued` for a message that was written, `duplicate` for one the deduplication window rejected (see [message deduplication](/use/examples/dedup)), and `buffered` when the broker is in maintenance mode and diverted the push to its disk spool.
...
<Aside type="caution">
A rejected ack still arrives as HTTP 200. `success` on the item is the only signal that the broker accepted it: a caller that checks only the status code reads an expired lease as a successful ack, and the messages redeliver.
</Aside>
