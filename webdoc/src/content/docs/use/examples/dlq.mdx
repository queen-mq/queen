...
- **It exists only for dead-lettered addresses.** A live message has nothing to replay and the call answers HTTP 404 with `No dead-letter row for this address — only dead-lettered messages can be replayed`.
