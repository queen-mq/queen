...
<Aside type="note">
Cross-broker ownership and forwarding arrive with the mesh work of this release line. Until they
land, a broker serves only the queues in its own memory, so a cell running more than one broker
should send one ephemeral queue's traffic to one broker. The SDKs already key their pops for that,
and a single-broker cell, a free-tier cell and the embedded engine are unaffected.
</Aside>
