...
<Aside type="caution">
Three verified behaviours carry over to PHP. `admin()->clearQueue()` and
...
collects only `queue`, `consumerGroup`, `limit` and `offset`. The response's `total` is the
size of the page rather than of the result set, so it cannot be used to page.
