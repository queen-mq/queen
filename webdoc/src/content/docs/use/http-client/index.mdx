...
# sourceOfTruth re-read 2026-09-11 (through bcea60c0 + working tree): auth.rs gained one read-only arm and main.rs three routes; the conventions this page teaches are unchanged.
---
...
`queued` (committed), `duplicate` (nothing was written, and `message_id` is the original
message's) or `error` (the broker refused that item's partition and wrote nothing for it). A `201`
does not by itself mean every item was stored: read every item's status.
