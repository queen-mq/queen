...
sourceOfTruth:
  ...
  - server/sql/procedures/020_streams_register_query_v1.sql
  - server/sql/procedures/022_streams_state_get_v1.sql
...
All four window kinds take the same option set: `gracePeriod`, `idleFlushMs`, and — to switch from processing time to event time — `eventTime`, `allowedLateness` and `onLate`.
