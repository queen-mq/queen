...
let handle = Stream::from(queen.queue("orders"))
    .filter(|r| r.number("amount").unwrap_or(0.0) > 0.0)
    .window_tumbling(60)
    .aggregate_count("count")
    .aggregate_sum("sum", |r| r.number("amount"))
    .to(queen.queue("orders.per_minute"))
    .run(&queen, RunOptions::new("orders.per_minute"))
    .await?;
```
...
The chain is immutable (every combinator returns a new stream) and it is finalised by `run`, which registers the query and starts the runtime loop. The same builder exists in four languages with the same operator set:
...
Rust names each aggregate field with its own call rather than passing a map of extractors, because the extractors have different signatures once they are typed. `aggregate_count("count").aggregate_sum("total", |r| r.number("amount"))` builds the same single aggregate the other SDKs express as one object, and fingerprints identically.

<Aside type="note">
The `config_hash` is computed the same way in all four SDKs: `sha256` over a JSON array of each operator's kind and structural configuration, with the keys in insertion order and user closures deliberately excluded. That is what lets the same query be redeployed in a different language without tripping the mismatch guard. The Rust implementation pins this with test vectors captured from the JavaScript one.
</Aside>
...
**Stream state is reclaimed explicitly.** Dropping the query (which cascades) or a `reset: true` re-registration are the ways to reclaim it; retention operates on segments and never touches `queen_streams.state`. A query with unbounded key cardinality (a session window keyed on something that never repeats, a sliding window over high-cardinality keys) grows that table until you intervene. Closing a window deletes its own row, so bounded-cardinality queries reach a steady state; unbounded ones do not.
