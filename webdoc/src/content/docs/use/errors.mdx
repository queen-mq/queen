...
Three parts of the contract are worth stating before the tables: a rejected acknowledgement arrives as HTTP 200, a rolled-back transaction arrives as HTTP 200, and an empty pop arrives as HTTP 204 with no body at all. On acks and transactions the per-item `success` field is the signal to branch on, and a 204 is bodiless, so read both alongside the status code.
