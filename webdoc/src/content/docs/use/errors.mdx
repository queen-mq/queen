...
The SDKs handle all three: the JavaScript, Python and Go clients raise on `success: false` for transactions and cycles, and normalise ack results per item — a truncated or misaligned ack response fails loudly rather than being misattributed to the wrong message.
