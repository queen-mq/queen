...
it a record in the same broker as the queue, written in the same transaction as the work it
...
two. None of these programs uses `forever`, which would leave rows in a shared broker that nothing
...
| the leader dies before a due timer fires | the timer is a record in the replicated state, so the next leader, or the same node once restarted, fires it. The fire is one entry of the log, so the compensation goes out once, and a cancel never answers `too_late` |
...
timer is delivered no earlier than its delay plus one fire step of the leader. And each run purges
what it wrote: the queues, the saga rows, and any timer still armed, because a pending timer is keyed
by name in a store of its own and deleting the queue does not reach it.
