...
structural: put the marker in the broker that already holds the queue, and write it in the same
...
outside the broker that would have to roll it back. What the pattern does is make the window as
...
The exception, and it is the only one, is the case where the effect is itself a write to this
broker: a push onto another queue, an acknowledgement, another KV write. Those go inside the
...
included: they live in the key/value store rather than in the queue, so deleting the queue does not
take them along.
