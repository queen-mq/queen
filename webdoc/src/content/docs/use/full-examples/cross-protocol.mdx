...
description: "Produce with a Kafka client, consume with the Queen SDK, against one set of messages. No connector, no mirror-maker, no second copy."
type: example
tier: use
sidebar:
  order: 7
...
The consumer reads over Queen's own API on port 6632. Both address **the same messages**, at the
same time, and neither knows the other exists.
...
converts and nothing is mapped at read time: they are one partition of one queue.
...
moves no cursor. Its progress lives in committed offsets in Queen's key/value store. So a topic
consumed only by Kafka clients shows `0.0 POP`, `0.0 ACK` and a `pending` count that never falls,
because only retention removes those messages. That is correct, not a stall.
