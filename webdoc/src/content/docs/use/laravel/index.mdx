...
Laravel worker may also pause while it still owns a prefetched tail. Queen therefore rejects a
supervisor configuration with `prefetch > 1` unless lease renewal is enabled.
...
worker if renewal can no longer finish safely. It cannot make external side effects exactly once,
so jobs must remain idempotent.
