...
sourceOfTruth:
  ...
  - clients/client-laravel/bin/queen-supervisor
...
verifiedBy:
  ...
  - clients/client-laravel/tests/SupervisorBinaryInstallerTest.php
---
...
- aggregate pool maxima must fit `process_limit`;
...
count against `process_limit`, so a reallocation cannot create a temporary process spike.
...
It uses capped exponential restart backoff. After repeated crashes it continues retrying at
`restart_backoff_max`; it does not have the Rust engine's open/probe circuit state.
...
`status --check` exits non-zero unless the current generation is live and healthy, which makes it
suitable for a local health check. Controls are fenced to the exact `instance_id`; a stale command
cannot apply to a replacement master.
