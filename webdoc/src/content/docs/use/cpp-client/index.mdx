...
the broker's array, one entry per item, each carrying a `status` of `queued`, `duplicate` or
`error`; supply your own `transactionId` and a retry inside the dedup window writes nothing a
second time. `pop()` leases a batch back.
