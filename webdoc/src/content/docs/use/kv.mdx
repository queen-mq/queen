...
`queen.kv` is a key/value store that lives in the same replicated state as the queues, and its
value is not that it stores keys. It is that a write to it can share the transaction with a push
and an ack: one entry in the broker's replicated log, applied whole or not at all.
...
easy to inflate. It is **not** exactly-once end to end: the charge is outside the broker, so a crash
...
`forever: true`. Zero or two of them is a `400`. The rule lives in the broker rather than in seven
clients, so it applies identically to the HTTP routes, the transaction wire and the embedded broker.

This is the difference between a store you can leave running and one you cannot. Stream state has no
expiry because a closed window deletes its own row; a KV key has no such event, so an optional TTL
would mean a store that grows in silence and an operator who pays for it.
...
<Aside type="caution">
The `version` handed to the loser is advisory. It is unique and opaque, and it is **not** a write
count. Compare it for equality, never for order, and never do arithmetic on it.
</Aside>

Two ordering rules complete the picture. **The transaction is the primary fence and `expect` is the
secondary assertion**: a state write sharing the transaction with an ack is refused with it when an
expired lease makes that ack fail, which a compare-and-set cannot do, because an `expect` on a version that
...
would put the loop back. Server side the arithmetic is exact decimal while the value fits a 128-bit
mantissa with at most 38 fractional digits, and falls back to a 64-bit float beyond that; typed SDKs
expose int64 and fail loudly rather than hand back a number that lost precision.
...
is not, because its cost is unbounded: the boundary is the cost rather than the kind of operation.
...
`POST /api/v1/kv` and nowhere else, and it requires a prefix: a namespace is not a list to enumerate.

The dashboard's KV browser is the one read that does enumerate one, and it is a different route for
that reason: `POST /api/v1/resources/kv/list`, read-only and viewer-visible, whose page may take an
empty prefix and whose rows include the keys whose expiry has passed but whose sweep has not happened
yet, labelled `expired: true`, so the console can grey them instead of hiding rows that still occupy
the namespace. It is a `POST` under the same privacy rule, applied to the one parameter that would
otherwise have to travel in a URL: its cursor is a key. `GET /api/v1/resources/kv/namespaces` is its
selector. [The KV routes](/reference/http/kv) has the field-by-field contract.

Page limits are clamped rather than refused, and `truncated` tells the truth. There is a byte ceiling
as well as a key ceiling, 4 MiB per call, because a thousand keys of 64 KiB is 64 MB and the real
resource is the byte. `after` is an exclusive keyset cursor, not an offset. Ordering is byte order,
so non-ASCII keys are not in the alphabetical order of a locale, and each page is its own snapshot:
good for compacting state, not for an exact count.

## Stream state is a different thing with a similar name

Both live in the broker's replicated state and only one of them is atomic with a stream cycle.

| | Stream state (`state_ops`) | `queen.kv` |
| --- | --- | --- |
| Identity | `(query_id, partition_id, key)` | `(tenant, namespace, key)` |
| Atomic with | The stream cycle: state, sink push and ack in one log entry | The transaction wire, when the write rides a bundle |
...
above 64 KiB. Keys are capped at 512 bytes, and at fewer when the tenant and namespace names are long,
because the store key carries all three and is itself bounded.

Values are never cached by the broker, at any TTL, and a read waits until the node answering it has
applied everything the cluster had committed when the read began. The first use of this store is an
idempotency marker, and a stale read of a marker says "not there" and performs the external effect
twice: a cache would make the transactional primitive eventually consistent in precisely the place it
exists not to be.
...
<CardGrid>
  <LinkCard
    href="/use/timers"
    title="Timers"
    description="The other half of the same release: a message promised now and delivered later, cancellable until it fires."
  />
  <LinkCard
    href="/internals/kv"
    title="KV internals"
    description="How KV entries are keyed, versioned and expired in the broker's replicated state."
  />
  <LinkCard
    href="/use/streams"
    title="Streams"
