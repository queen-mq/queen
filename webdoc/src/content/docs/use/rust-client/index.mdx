...
carries a `status`: `Queued`, `Duplicate` or `Error`. The enum also has `Buffered` and `Failed`,
which the 2.0 broker does not return. A `transaction_id` you do not supply is minted client-side
as a UUIDv7, unique per call and therefore not idempotent.
