...
const DEFS: [Def; NTABLES] = [
    ...
    TableDefinition::new(TABLES[9]),
    TableDefinition::new(TABLES[10]),
    TableDefinition::new(TABLES[11]),
    TableDefinition::new(TABLES[12]),
    TableDefinition::new(TABLES[13]),
];
...
impl Engine for Redb {
    fn config_note(&self) -> String {
        "redb 2.6.3, Durability::None between durable points".into()
    }
