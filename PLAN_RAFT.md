...
| D11 (ratified: openraft) | Consensus behind a `Replicator` trait (§12.1). Phases 1–2 use `LocalReplicator` (no network). Library: **openraft at git rev `54094270ede0b8a2eb6ed6ae990edc6ca19d98ec`** (0.10.0-alpha.34 plus the GH#2095 `LogEntryDiscarded` fix) with **raft-log 0.4.6** as the log store, re-confirmed at G3 against whatever 0.10 release exists then; fallback raft-rs. | S3 (test/raft/spikes/s3-consensus/MEMO.md): raft-log passes openraft's log-store conformance suite; on the VM, 3 ms commit p50 for 64 KiB entries, a new leader 3.0–3.4 s after kill -9 with 0 acknowledged writes missing in 4 runs, leadership transfer in 28 ms, linearizable reads ~1.1 ms, a 1 GiB snapshot installed at 398 MiB/s, the wiped-voter procedure verified both ways. |
| D12 (ratified: framed TCP; S4 measured half the leader CPU per message of HTTP/1.1 at 50k msg/s, 1.35 vs 2.77 µs, and 5% fewer bytes) | Raft traffic and forwarding use a dedicated binary port `QUEEN_RAFT_PORT` (6634) with length-prefixed frames. Authentication is mutual: an HMAC with `QUEEN_RAFT_SECRET` over both sides' nonces, binding cluster_id and the from/to node ids; then TLS (rustls is already a dependency) or a MAC on every frame, so frames cannot be injected after the handshake. A node without a matching IDENTITY rejects Raft RPCs (the enforcement point of I9). Multi-node mode refuses to start without a secret. Confirmed by spike S4. | pgless's replica lane ran wide open when its secret was unset (U19). |
| D13 | Receivers hold requests up to `QUEEN_RAFT_HOLD_MS` (8000) while no leader is known, then answer 503 `no_leader`. | JS, Python and PHP consumers exit on a lasting 5xx; Kafka groups stall after 10 s; the proxy passes 502/504 through. |
| D14 | Heartbeat 100 ms, election timeout 1000–2000 ms, pre-vote on, quorum check (leader lease) on. | Cloud networks; etcd's defaults are 100 ms / 1000 ms. openraft's defaults (50 / 150–300 ms) are LAN-tuned. |
| D15 | Local stale reads for fetch, partitions/changed, browse, DLQ lists, stats, analytics, traces and timer lists. Linearizable reads (read barrier, §9.4) for KV gets and lists and for `POST /streams/v1/state/get`; they judge expiry at the leader-stamped `now` the barrier returns. | The Kafka, SQS and S3-sink fences read KV. |
...
| D21 | Each data dir carries `IDENTITY {cluster_id, node_id, generation, disk_uuid}`. A node never votes with a dir that does not match its membership record. A wiped disk rejoins only as a new member (remove, add as learner, promote). | openraft calls a wiped voter "undefined behavior" and has no release-build guard; pgless C01/C22. |
...
- Linearizable reads (KV gets and lists, and any endpoint a facade uses as a
  ...
  - the leader batches barrier requests every ≤ 2 ms into one library
    linearizable-read call (openraft sends one RPC round per call, so batching
    is the application's job) and answers `{index, now_us}`;
...
3. Truncate every segment file to the length recorded in that reopened state
   ...
   directory and install a snapshot from the leader (raft3), or refuse to
   start with a clear error (raft1) (I11).
...
binary as the oracle until G2 (D22).
