...
func stageOne(ctx context.Context, client *queen.Queen, count int) error {
	for handled := 0; handled < count; handled++ {
		...
		// derived message are one entry in the broker's replicated log, so
		// they either both happen or neither does. Ack first and crash, and
		// the invoice is lost; push first and crash, and the order is invoiced
		// twice on redelivery. This is the shape that has neither failure.
		//
