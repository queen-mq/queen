...
async fn run() -> Result<usize, String> {
    ...
    // queue already has, and QueueOptions skips its None fields, so this call
    // sends the two below and nothing else. The queue name is unique per run, so
    // nothing is being kept here and the rest lands on its default.
    queen
        .queue(&queue)
