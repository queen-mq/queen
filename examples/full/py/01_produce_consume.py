...
async def main():
    ...
    async with Queen(BROKER_URL) as client:
        ...
        # 1. Create the queue.
        # ------------------------------------------------------------------
        # /api/v1/configure merges: an option the body does not carry keeps the
        # value the queue already has, and an explicit None restores that one
        # option's default. This client merges its own nine-key default bag into
        # every .config() call, so those nine are sent whether you name them or
        # not; dedup_window_seconds below is not one of them.
        created = await (
            client.queue(QUEUE)
            .config(
                {
