...
| `fire` | delete e push in una transazione | `stale` (token non corrispondente), `duplicate` (il txn e' gia' nel log) |
...
**Il task non viene spawnato se entrambe le feature sono spente.** Con `QUEEN_SWEEPER=true` di default e i due flag a `false`, ogni installazione esistente pagherebbe per sempre una `log_timers_due_v1` al secondo per broker (LATERAL a 64 seek piu' una count), una `kv_expire_step_v1` al secondo, uno slot `Lane::Maint` e una connessione per ciclo, su due tabelle vuote. Su un free tier a 2 core con ceiling misurato attorno a 480 msg/s e' rumore misurabile che nessun cliente ha chiesto. Quindi: `if !kv_enabled && !timers_enabled { return; }`, la forma di kill switch gia' usata altrove in `main.rs`.
...
1. Due rami nuovi nel `match ty`: `"kv"` e `"timer"`, che accumulano in due vettori dedicati e registrano l'ordinale piatto.
2. **La mappa `flat index → (array, ordinale)`**, mantenuta in un vettore parallelo, e lo **scatter inverso** dei risultati dallo spazio per-array allo spazio piatto di `results[]`, che e' il contratto che i client leggono oggi.
...
4. Il boot **rifiuta di partire** con `QUEEN_TENANCY_HEADER=1` e `QUEEN_KV_ENABLED=1` senza un'affermazione esplicita dell'operatore che c'e' un proxy davanti. Il precedente e' `env_bool`, che su un valore non riconosciuto chiama `obs::fatal()`: per un interruttore di sicurezza e' il comportamento giusto, e `QUEEN_KV_ENABLED=ture` non deve accendere niente ne', peggio, spegnere qualcosa che era acceso.
...
| feature spenta a runtime | 503 piu' `Retry-After` | `kv_disabled` |
| feature spenta da env | 404 | fallback JSON: la superficie non esiste su questa cella |
...
| `deploy/state.mdx` | operatore, blocca la **cella condivisa** | i quattro livelli di interruttore e cosa risponde ognuno (404, 503, 403, 403), le tre difese dell'endpoint, l'ordine di rilascio, e la condizione verificabile per la cella condivisa |
...
**`status: preview`** su tutte le pagine kv e timers finche' gli interruttori sono `false` di default. E' esattamente cio' per cui il campo esiste, ed e' l'unico modo onesto di pubblicare una superficie spenta.
...
| `reference/errors.mdx` | il `required` perso e' un `RAISE` 23514 tradotto in **HTTP 200**: e' l'unica eccezione alla regola che la pagina enuncia, e va scritta li' o la pagina e' sbagliata. Piu' i codici nuovi e la distinzione 404 quando env spento contro 503 quando runtime spento |
...
| push che risponde `duplicate` | il `txn` fisso e' gia' nel log: quel timer e' gia' stato consegnato. Si cancellano i timer duplicati, non si pusha, e il resto del gruppo torna claimabile per essere ri-impacchettato senza di loro. Il gruppo avanza a ogni passata | `{result="duplicate"}` |
...
| sweeper fermo (flag spento, tutte le repliche giu') | nessuna perdita: `deliverAt` e' "non prima di", e al ritorno si riparte dal piu' vecchio. Sotto `panic = "abort"` il caso "il task e' morto e il broker serve" non esiste, perche' un panic abbatte il processo e k8s riavvia | `queen_timers_due_backlog`, warn a 60 s continui, page a 300 s |
...
I flag di runtime vivono in `queen.system_state`, sulla forma gia' usata da `maintenance_mode` e `pop_maintenance_mode`: flag in-process autorevole sul percorso caldo, riga in DB come specchio best-effort per propagazione e restart, rilettura fresca sul GET. Un incidente alle tre di notte non si risolve con un rollout. Precedenza fra i due livelli, e i due status sono diversi apposta: env `false` uguale **404** (la superficie non esiste su questa cella), env `true` piu' system_state `false` uguale **503** con `Retry-After` (interruttore operativo, temporaneo, e il client deve ritentare).
...
| **Parity gate** | matrice completa delle suite con **entrambi i flag OFF**, bit-identica al baseline. Il default-OFF non e' una promessa, e' un test | F1, ogni fase |
...
| fase | contenuto | spedisce al buio? | criterio di fatto | stima |
|---|---|---|---|---|
...
| **F1** | `024_kv.sql` e `025_log_timers.sql`, tabelle e helper puri, riga in `PROCEDURES`, check versione PG al boot. **Nessun lettore** | **si'** | doppio boot verde su vergine e su popolato; parity suite bit-identica; `42P22` verificato sul rig; unit degli helper | 4 gg |
| **F2** | `kv_apply_v1` completa, rotte KV, pool dedicato, token bucket, single-flight, regole `auth.rs` piu' fingerprint. `QUEEN_KV_ENABLED` ancora `false` | **si'** con flag off | test di semantica KV verdi; grep meccanico verde; con flag off le rotte danno 404 e la suite e' bit-identica | 8 gg |
| **F3** | `025` completa (apply, due, claim, fire, fail, dlq, peek, list), `sweeper.rs`, estrazione di `pack_segment` e del classificatore, metriche. `QUEEN_TIMERS_ENABLED` ancora `false` | **si'** con flag off | fault injection completa; isolamento due tenant; perf gate sweeper, incluso il costo a tabella vuota | 10 gg |
| **F4** | **Il centro di rischio.** Innesto nel wire (step 0 e 0b), demux e mux in `handlers/data.rs`, `txn_fail_body` esteso, instradamento dei bundle solo-KV | no: da qui in poi il percorso caldo e' toccato | test di concorrenza sull'ordine di lock verde; test transazionali verdi; **perf gate con CPU per messaggio entro l'1%** e zero query nuove in `pg_stat_statements` | 6 gg |
...
**Cosa e' utile anche se il resto slitta:** F0 e F1 lo sono, perche' il DDL canonico e la dimostrazione dell'ordine di lock valgono da soli e chiudono per sempre la classe di contraddizioni che ha bloccato questo piano. F2 e' utile senza F3 e viceversa: le due feature sono indipendenti sul percorso di rilascio, e un cliente puo' volere solo i timer.

**Cosa spedisce al buio:** tutto fino a F3 compresa. Le tabelle esistono, le SP esistono, nessuno le chiama, e le celle cloud negano gia' le rotte fail-closed senza toccare il proxy.
...
**Ordine di accensione**, che e' diverso dall'ordine di sviluppo:

| passo | cosa e' vivo | dove si accende |
|---|---|---|
| 1 | broker completo, entrambi i flag `false` | nessuno; le rotte non sono nemmeno registrate |
| 2 | flag on su cella self-hosted o dedicata **senza** proxy | chi si amministra da solo: l'operatore e' il cliente stesso |
| 3 | F8 P1 (gate) | un cliente **dedicato** dietro proxy, per invito, reversibile con un UPDATE |
| 4 | F8 P2 (misura) piu' un soak completo | ancora dedicato, ma ora la quota storage e' onesta |
| 5 | F8 P3 e P4 | **cella condivisa** |

**La regola:** niente cella condivisa finche' la misura di P2 non e' in produzione da un soak intero, con `kv_expired_not_pruned` sotto soglia per tutta la durata. Un limite senza misura e' un limite finto.
...
### 17.1 Spegnere i flag

| scenario | comportamento deciso |
|---|---|
| `QUEEN_TIMERS_ENABLED` a `false` con timer pendenti | i timer **restano in tabella e non partono**. Non si perdono e non si drenano di corsia. Riaccendendo, il drenaggio riparte dal piu' vecchio. Il boot **logga un WARN** che nomina il conteggio pendente, cosi' lo spegnimento non e' silenzioso. Non si rifiuta lo spegnimento: un interruttore d'emergenza che si rifiuta di scattare non e' un interruttore |
| `QUEEN_KV_ENABLED` a `false` con chiavi vive | le chiavi restano, e **non vengono piu' potate** (lo sweeper non gira se entrambi i flag sono spenti). Il WARN di boot nomina il conteggio. Se la cella spegne il KV e tiene i timer accesi, la potatura continua, ed e' il caso buono |
| spegnere entrambi con tabelle non vuote | e' l'unica combinazione che accumula spazio in silenzio, ed e' quella che il WARN deve gridare piu' forte |
| kill switch di runtime | reversibile in un secondo, senza rollout, senza perdita (§12.1) |

**Non esiste un percorso di rollback che perda dati**, ed e' una differenza sostanziale rispetto ad archiviare blob: qui nessuna colonna esistente viene svuotata, nessun dato viene spostato altrove, e spegnere non rende niente illeggibile.
...
| **flag OFF di default** | un default si cambia da `false` a `true` dopo una campagna di misura, il contrario e' una regressione per chi lo usa gia' | deciso: OFF |
...
4. **Il numero di worker autovacuum** occupati dalle tabelle nuove in stato stazionario, contro `autovacuum_max_workers` (default 3). Se mordesse, il piano deve dire di alzarlo quando la cella accende la feature, altrimenti il primo sintomo e' un rallentamento del log senza colpevole.

Piu' due criteri di accettazione che sono numeri, non giudizi: **CPU per messaggio entro l'1%** con i flag spenti, e **tempo di tenuta p99 del lock `log_consumers`** invariato con la feature accesa.
...
| S8 | Il fuoco non rilascia le righe di un segmento `stale` | **corretto**, §6.2 punto 3, con rilascio simmetrico anche nel ramo `duplicate` |
...
| S6 | Sweeper acceso di default con entrambe le feature spente | **corretto**, §7.1: non viene spawnato, piu' backoff a tabella vuota |
...
Cinque erano aperte. **Le tre porte a senso unico sul wire sono state chiuse da Alice il 2026-08-17** (20.1, 20.2, 20.3) e restano qui come registro, non come domande. Ne restano tre: due reversibili (20.4, 20.5), e **una nuova porta a senso unico aperta proprio dalla ratifica di 20.1** (20.6, l'unita' di durata sul wire dei timer), che va chiusa insieme a quella o il prodotto resta con due convenzioni non dichiarate. Ognuna ha un default proposto e una scadenza.
...
Decisione di Alice, 2026-08-17. Ogni reschedule conia un `txn` nuovo, cioe' e' un messaggio nuovo. Cosi' "questo timer, riprogrammato, ha consegnato questo messaggio" e' rispondibile senza ambiguita', `txn` resta **obbligatorio** nell'op `schedule` (la forma del wire non cambia), e un payload sostituito non condivide mai un identificatore con quello che ha rimpiazzato. Il `txn` fisso resta la rete di dedup del **singolo** tentativo di fuoco, che e' il caso per cui esiste. Da scrivere in documentazione, perche' e' il corollario che morde: **la rete di dedup non attraversa un reschedule**, quindi riprogrammare un timer gia' partito non e' idempotente rispetto al messaggio gia' consegnato.
...
**20.4 `QUEEN_KV_ENABLED` e `QUEEN_TIMERS_ENABLED` restano `false` per il GA. Scadenza: fine F5.**
Proposta: **si', OFF di default**, con accensione per cella. Tre argomenti. La quota di occupazione e' morbida per costruzione e si appoggia a una misura che il giorno uno non esiste in produzione, quindi acceso di default significa che il primo tenant che tratta il KV come Redis degrada WAL e autovacuum di una cella condivisa prima che qualcuno possa vederlo. L'asimmetria: un default si cambia da `false` a `true` dopo una campagna di misura, il contrario e' una regressione per chi lo usa gia'. E la condizione di accensione e' esplicita e verificabile, non un giudizio: la misura in produzione da un soak intero con `kv_expired_not_pruned` sotto soglia (§16). Se preferisci accendere il KV di default sul self-hosted, dove l'operatore e' il cliente stesso e non ci sono vicini, e' una variante difendibile e va detta adesso, perche' cambia il testo di quattro pagine.
