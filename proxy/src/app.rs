//! The proxy's composition: the standalone binary's boot (`main_standalone`)
//! and, for the single binary, the pieces the broker assembles in-process.
...
use crate::{auth, cache, config, console, db, limits, meter, obs, oauth, operator, registry, webapp};
...
/// The standalone `queen-proxy` binary.
pub fn main_standalone() {
...
async fn async_main(worker_threads: usize) {
    ...
    // Upstream to the cell broker: pooled HTTP/1.1, TCP_NODELAY on (the proxy
    // relays small bodies and Nagle would hold them ~40ms), 64 idle per host.
    let upstream = crate::upstream::Upstream::http();
    let store = match &db {
        Some(pool) => crate::store::Store::Pg(pool.clone()),
