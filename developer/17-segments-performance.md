...
10-minute soak (1500/850, fresh PG, `completedRetentionSeconds=300`):

```
[15:22:16] push= 884423/s pop= 883533/s
[15:23:16] push= 868933/s pop= 869107/s
[15:24:16] push= 836397/s pop= 836842/s
[15:25:16] push= 760138/s pop= 759742/s
[15:26:16] push= 708707/s pop= 708455/s
[15:27:16] push= 657945/s pop= 657773/s
[15:28:16] push= 671953/s pop= 672058/s
[15:29:16] push= 682680/s pop= 683125/s
[15:30:16] push= 612572/s pop= 611818/s
[15:31:16] push= 508762/s pop= 509580/s
[final] pushed=431552300 popped=431522000 pushErr=0 popErr=8
```

- **431.5M messages each way in 600s → ~~719k push/s + ~719k pop/s sustained
average (~~1.44M combined)**, 0 push errors, 8 pop errors, **zero deadlocks**
in the entire soak, consumers caught up at the end (deficit 30k ≈ one pop
batch per consumer).
- Throughput declines from ~884k to ~510-680k per side as stored data grows
(DB reached 12 GB / 229k live segments by the end): PG waits shift toward
`LWLock:BufferContent` + `Lock:extend`, PG CPU rises, broker stays ~75-80%
busy. Short 120s runs therefore overstate steady-state throughput.
- The decline is a **data-volume / retention-sweep effect, not the deadlock**
(zero occurrences in 10 minutes). This is the open retention/dedup work:
pending-message retention, sweep cost, and `seg_dedup` partitioning are the
next levers for flat long-run throughput.
...
- **NEXT (planned): multitenant-shape benchmark — 2000 queues × 100 msg/s**
  (same aggregate bandwidth as today's runs, completely different shape for PG:
  catalog size, per-queue candidate scans, per-group watermarks, thousands of
  parked long-polls). Goal: validate the 2000-tenant density assumption before
  any SaaS pricing commitment. Needs a goload variant that spreads load across
  N queues instead of N partitions of one queue.
  - **Second leg — idle cost.** Production data (SmartChat et al., 2026-07):
    92 queues / 134k partitions / 65 groups / 200 consumers, ~10-50 msg/s,
    50% empty pops → **0.5-1 PG core of pure baseline load**. Per-message cost
    is ~4 orders of magnitude worse than the bench because empty wildcard pops
    scan `seg_partitions`⋈`partition_consumers` at up to 1 Hz per consumer
    (`POP_WAIT_MAX_INTERVAL_MS=1000` backoff cap). Idle-tenant cost, not
    throughput, may be the real density limit. To test: mostly-idle queues,
    parked consumers, high partition counts, 7d retention. Candidate quick
    win to validate: raise `POP_WAIT_MAX_INTERVAL_MS` to 10-30s (push-wake
    makes the periodic re-poll a safety net only); confirm top query first
    via `pg_stat_statements` in prod.
