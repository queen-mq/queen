//! The boot: environment configuration, then the listener.
//!
//! Shared by the two ways this facade runs. The `queen-kafka` binary resolves a
//! [`Config`], builds an HTTP [`queen::HttpQueen`] for `QUEEN_URL` and calls
//! [`serve`] until SIGTERM. A broker that runs the facade IN-PROCESS (single
//! binary, server/src/kafka_inproc.rs) resolves the same [`Config`] from the same
//! environment, hands [`serve`] a client whose transport is its own router, and
//! stops it on its own shutdown. Nothing here exits the process: a failure is an
//! `Err` for the caller to decide about, because in-process the caller is a
//! broker that must keep serving.
//!
//! Spec: PLAN_QUEEN_KAFKA.md (repo root); the protocol lives in the rest of the
//! library.

use std::sync::Arc;
use std::time::Duration;

use crate::cluster::{self, registry, Cluster, ClusterState};
use crate::coordinator::{Coordinator, GroupConfig};
use crate::handlers::metadata;
use crate::txn;
use crate::{conn, queen, tls, Facade, Policy};
...
pub struct Config {
    ...
    /// The transaction stage's caps (M9, [`crate::txn`]). Every one of
...
impl Config {
    ...
    pub fn resolve(get: impl Fn(&str) -> Option<String>) -> Result<Config, String> {
        let env = |k: &str| {
            get(k)
        ...
        let max_connections = match env("QUEEN_KAFKA_MAX_CONNECTIONS") {
            None => crate::DEFAULT_MAX_CONNECTIONS,
        ...
        let cluster =
            match env("QUEEN_KAFKA_CLUSTER") {
                None => crate::handlers::metadata::CLUSTER_ID.to_string(),
...
impl Config {
    /// `QUEEN_URL`, normalized.
    pub fn queen_url(&self) -> &str {
        &self.queen_url
    }

    /// `QUEEN_KAFKA_ADDR`: what the listener binds.
    pub fn listen_addr(&self) -> &str {
        &self.listen_addr
    }

    /// `QUEEN_KAFKA_ADVERTISED_ADDR`, as the `host:port` clients are told.
    pub fn advertised(&self) -> String {
        format!("{}:{}", self.advertised_host, self.advertised_port)
    }

    /// Whether `QUEEN_KAFKA_NODE_ID` put this facade in cluster mode.
...
/// bound — is returned instead, with the sentence the binary printed.
pub async fn serve(
    cfg: Config,
        ...
        Some(id) => {
            ...
            let version =
                match registry::claim(api.as_ref(), &state, cfg.queen_token.as_deref()).await {
                    ...
                    Err(registry::Refused::Taken(message)) => return Err(message),
    ...
    let listener = match tokio::net::TcpListener::bind(&cfg.listen_addr).await {
        ...
        Err(e) => {
            return Err(format!(
                "cannot bind QUEEN_KAFKA_ADDR={}: {e}",
                cfg.listen_addr
            ))
        }
    };
...
mod tests {
    ...
    fn the_connection_cap_defaults_and_is_loud_when_wrong() {
        ...
        assert_eq!(cfg.policy.max_connections, crate::DEFAULT_MAX_CONNECTIONS);
