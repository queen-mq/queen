...
- **7.5 Durable A/B gate — the acceptance bar** — reuse `kv-timers-gates/perf-gate-hotpath.sh` conventions (M11): durable-only load, before/after images, CPU µs/msg within `TOLERANCE_PCT=1.0`, `GATE_REPS>=3`, and the pin *zero new `pg_stat_statements` entries with calls>0 on a durable-only run*. Trivially green if §5.2 held — which is the point of gating it.
