...
//
//	-mode cloud           MULTI-TENANT workload for the queen_proxy campaign
//	                      (2026-07-29). Drives the proxy (every simulated tenant
//	                      is a REAL tenant: own cluster, own api key, own Host)
//	                      or the broker directly, optionally with every tenant on
//	                      the SAME queue + consumer group (the shared-cell shape),
//	                      with per-message (tenant, seq) delivery accounting for
//	                      loss / duplication / cross-tenant leakage, error counts
//	                      by kind AND proxy code, and per-run JSON+CSV artifacts.
//	                      See cloud.go. `goload -mode cloud -h` for its flags.
//
//	-mode provision       Create N real tenants via queen_proxy.bootstrap_tenant
//	                      (psql on the cell's pxdb) and cache their plaintext api
//	                      keys to a file so re-runs never re-provision. See
//	                      tenantfile.go.
//
...
func main() {
	...
	case "cm":
		runCMMode(os.Args[1:])
	case "cloud":
		runCloudMode(os.Args[1:])
	case "provision":
		runProvisionMode(os.Args[1:])
	case "order":
		runOrderMode(os.Args[1:])
	case "max", "":
		runMaxMode(os.Args[1:])
	default:
		fmt.Printf("goload: unknown -mode %q (want: max | app | openloop | cm | cloud | provision | order)\n", mode)
		os.Exit(2)
	}
}
