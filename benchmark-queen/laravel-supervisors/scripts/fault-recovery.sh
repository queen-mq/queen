...
The test fixes BENCH_PROFILE=fixed, QUEEN_PREFETCH=1 and QUEEN_ACK_BATCH=1.
...
write_protocol_metadata() {
    python3 - "$OUTPUT_DIRECTORY" "$campaign_id" "$REPOSITORY_ROOT" "$ENGINES_CSV" \
        ...
        "$WORKER_TIMEOUT" "$RETRY_AFTER" "$KILL_DELAY_MS" "$RESPAWN_TIMEOUT" \
...
(
    ...
    cpu_iterations, job_tries, worker_timeout, retry_after, kill_delay_ms,
...
write_lane_summary() {
    ...
    python3 - "$summary_lane" "$summary_engine" "$summary_run_id" "$summary_app_id" \
        "$JOBS" "$JOB_TRIES" "$WORKERS" "$SLEEP_MS" "$KILL_DELAY_MS" \
...
(
    lane_raw, engine, run_id, app_id, jobs_raw, job_tries_raw, workers_raw,
