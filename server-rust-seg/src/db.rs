...
// Push one segment. `blob` is the raw zstd bytes; the wire wrapper
// (queen.seg_push_segment_wire_v1) takes the blob as base64 TEXT and decodes it
// SQL-side (decode(...,'base64')), so we base64-encode here. Returns the SP
// result JSON as text.
pub async fn push_segment(
    ...
    let row = client
        .query_one(stmt, &[&queue, &partition, &metas_json, &blob_b64, &msg_count])
        .await?;
    Ok(row.get(0))
}
